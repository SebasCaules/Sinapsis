...
| S-25 | `chi2Inv` (`packages/runtime/src/math.ts`) expande el corchete sin tope (`while (chi2CDF(hi,k) < p) hi *= 2`): con un `p` patológico es un bucle infinito; ponerle la guarda de `tInv` (`hi < 1e300`). | packages/runtime math.ts | reporte Y5 | Sprint 4 |
| S-26 | `unloadBundle` no deshace el merge de `study-data.json` en `App.STUDY` (Object.assign sin estado previo); si dos bundles aportan `study-data` se pisan. `propose` deja el archivo de propuesta sin seguimiento si falla el commit (el reintento lo reescribe). | packages/runtime loader.ts, packages/cli propose.ts | reporte Y5 | Sprint 4 |
| S-27 | El plan de Proba en «Cursada + final» suma 57 tareas contra 66 del original (9 se pierden en el conversor: revisar `convert-study-data.mjs` contra `ROADMAP` de `study-data.js`). | examples/proba conversor | verificador del plan | próxima ola de datos |
| S-28 | `PlanDate` valida formato y no calendario (`2026-13-45` se guarda); el `DatePicker` ya no deja elegirla, pero el API la acepta. Rechazar días imposibles en el contrato. | packages/contract | verificador del plan | Sprint 4 |
| S-29 | Actividad local (racha, última página leída) vive en localStorage (`activity.ts`); promover a `StudyState` cuando se quiera compartir entre dispositivos. Lo mismo para `toolState` de los bundles (estado de ejercicios, «mi hoja»). | contrato, API, web | brechas (inicio, no portado) | Sprint 4 |
| S-30 | Paleta ⌘K: consumir los proveedores de búsqueda que registran los bundles (`App.registerSearchProvider`, ya en el runtime) para buscar fórmulas (grupo «Fórmulas», destino `?f=`) y vistas de herramienta por sección. | apps/web SearchPalette | brechas shell-05 / noportado-34 | siguiente ola |
| S-31 | Puntos de extensión de la vista de división para los bundles: `App.registerDivisionSection` (colecciones de ejercicios al pie de «ver toda la unidad») y `App.registerSequenceSteps` (pasos de ejercicios en la barra de unidad y «+N ejercicios»). Diseño de API en el reporte del port. | runtime, DivisionView, ReaderView | brechas noportado-17/18 | siguiente ola |
| S-32 | `routes.tool` con el argumento como segmento (`/m/<s>/t/<vista>/<arg>`) en vez de `?arg=`; `ToolManifest.shortcuts` para listar ⌘J en el diálogo de atajos; hueco en `Crumbs` para el botón de volver del taller; grupos temáticos de divisiones para la leyenda del grafo. | contrato, web | brechas herr-14/17/18, grafo-11 | Sprint 4 |
| S-33 | El conversor `convert-study-data.mjs` tiene que regenerar `kits.json` (lanzadores con parámetros, icono y color) y `plan.json` (simulacros `kind: "tool"` con `query`) tal como quedaron editados a mano; y `Kit`/`PlanTask.query` documentados en la skill. | examples/proba, skills | brechas kits-05, noportado-22 | siguiente ola |
| S-34 | El quiz declarado de Proba sigue en 15 preguntas; el bundle arma 43 (con las 28 autogeneradas de E[X] y V(X)). Generar las 28 en el conversor como segundo quiz. | examples/proba | brechas quiz-02 | siguiente ola |
| S-35 | `SessionView.test.tsx` «Espacio sobre la tarjeta enfocada la da vuelta UNA sola vez» falla de forma intermitente bajo carga (local con `pnpm test` completo y en el CI de #6: espera «Ocultar la respuesta» y lee «Ver la respuesta»); a solas pasa 5/5. Estabilizar (esperar el re-render con `findBy`/`waitFor` o fijar el foco antes de la tecla). | apps/web SessionView.test.tsx | orquestador | Sprint 5 |
...
| S4-G Etiqueta de tipo única (`TypeTag`) en todas las vistas (pedido del usuario) | DONE | componente del orquestador (`components/TypeTag`); adopción por el agente G (Opus) en portada, índice, catálogo, inicio, favoritos, kits, paleta y grafo |
| S4-H Marco de página compartido (`PageFrame`): lector y vista de ejercicios con la misma hoja, cabecera y barra; tooltips en los pasos de ejercicios (pedido del usuario) | DONE | agente H (Opus): `PageFrame`, modo genérico de `PageTip` (`data-tip-*`), `ToolView.frame: "page"`, `model.stepForTool`, N0-64; verificado por el orquestador en Chrome. Gates: 1 054 tests, E2E 94/3 (Proba) y 78/19 (demo) |
...
| S4-P Push a `origin/main`, fuente de Pages = GitHub Actions, primer deploy | DONE | Pages ya estaba en `build_type: workflow` (lo activó la sesión de cripto al mergear el PR #1). El 2026-09-07 el orquestador empujó `main` (c6076a6..34b157f: auditoría de temas) por orden del usuario («hace todo acá»); corrida `pages.yml` 34070837069 en verde; verificado en `https://sebascaules.github.io/Sinapsis/`: CSS publicado con `--border-3`/`--hover-wash` y sin `--grain-22`, claustro con la capa de grano encima y `--border #4a4028`. |
| S4-T Auditoría de los tres temas y corrección (pedido del usuario, 2026-09-06: «esos contenedores en modo oscuro se ven mal; revisa todos los temas, combinaciones prohibidas») | DONE | Orquestador: grano de Claustro como capa global (N0-65, quita 29 reglas `--grain-22`) y tokens N0-67 (`--border-3`, `--track` ≥ 3:1, `--hover-wash`/`--active-wash`, bordes de Claustro, `--text-3`/`--surface-3` de los claros, `--ink-mix` 72 %). Captura propia de 37 vistas × 3 temas (`capture.mjs`, Playwright contra :5173) con censo de contraste WCAG en el DOM; 7 auditores Opus por área de CSS → 114 hallazgos (4 P0, 42 P1); adjudicación (5 rechazados: C-11/D-01 por N0-55, G-16, C-05/D-15 resueltos por token); 7 correctores Opus con ownership disjunto → 115 aplicados; un agente igualó las dos copias gemelas del vocabulario de los bundles de Proba (la de `proba-exercises` pisa a la de `proba-tools`) y quitó la opacidad en línea de la leyenda de figuras. Gates: typecheck limpio, web 700, runtime 178, censo en cero fallas en los tres temas; E2E al cierre (ver veredicto). Commits `3f20156`, `dfc1f7c`, `c20a224`, `f04f438`…`f01a715`. |
| S4-U Cierre del trabajo de la sesión de cripto (pedido del usuario, 2026-09-07: «fíjate en qué quedó la otra sesión y termina el trabajo») | DONE | Dos agentes Opus respondieron las revisiones: PR #5 Mermaid (`suppressErrorRendering`, `isDrawn`, test con la librería real `Mermaid.real.test.tsx`, N0-69) y PR #2 adjuntos (nombre publicado validado entrada por entrada + realpath, `svg` excluido con `asset-unsupported`, `asset-too-big` con página, N0-68); el orquestador los revisó en worktrees (gates verdes), mergeó en `main` (`01ee8a9`, `14a01c7`), cerró propuestas/INBOX/DECISIONS (`bbb403a`), publicó cripto desde el vault con `sinapsis publish` (PR #6, 179 páginas, 50 adjuntos, sin advertencias) y lo mergeó (`cafe3dd`). Gates en `main`: typecheck, 1 108 pruebas. CI de #6: un `gates` verde y uno rojo por la prueba intermitente `SessionView` «Espacio… UNA sola vez» (S-35). Limpieza: stash de cripto descartado (superado por el publish), ramas locales `preview/cripto-20260906` y `subject/cripto-preview` borradas. |
| S4-W Sin cajón «Transversales» y páginas sueltas arriba del índice (pedido del usuario, 2026-09-07: «eliminar la pestaña de transversales; índice y registro por los atajos del rail; el formulario de Proba arriba de todo como página independiente; en Cripto el mismo criterio») | DONE | orquestador: N0-74. Contrato: `effectiveDivisions` ya no sintetiza `meta`; `wiki.standalone: Slug[]` (default `[]`) y `standalonePages`; rótulos `LOOSE_DIVISION_*`. Web: el modelo expone `standalone`, `loose`, `catalogDivisions` y `bucket`; el índice dibuja las sueltas arriba del árbol (`standalone-row`); lector sin vecinos ni posición para páginas sin división; catálogo, favoritos y grafo agrupan las sin división bajo «Sin división» (sin portada). CLI: aviso por página sin división fuera de `wiki.standalone` y por entrada inexistente; informe «Sin división (índice, registro, sueltas)». Configs: Proba `["formulario-maestro"]`, Cripto `[]` (repo y vault). E2E: `divisionsVisible` sin el término `meta`, prueba nueva de las filas sueltas. Docs: contratos 01 (§7.1), 02 (§5), 06, skill y referencia, PROMPT-migracion. |
