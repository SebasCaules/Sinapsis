...
enlazada (`ln -s "$SINAPSIS_HOME/skills/sinapsis" ~/.claude/skills/sinapsis`), y el stack de
desarrollo levantado (`pnpm dev` en la plataforma: API en :3000, web en :5173) con su
`apps/api/.env` (`SYNC_TOKEN`).
...
`/sinapsis` y sin tocar el repositorio de la plataforma.
...
- Los contratos que mandan (léelos ANTES de empezar, en ese orden):
  ...
  `03-estudio.md`, `04-herramientas-y-figuras.md`, `06-skill-y-agentes.md`,
  `07-propuestas.md`. Y la skill: `$SINAPSIS_HOME/skills/sinapsis/SKILL.md`.
- Referencia de una materia ya migrada: `$SINAPSIS_HOME/examples/proba/` (config,
  `estudio/`, `tools/`).
...
- Cada paso termina con su verificación (`validate`, `sync --dry-run`, navegador). No das
...
- Cero wikilinks rotos (los verifica el `sync --dry-run`).
...
5. `pnpm --dir "$SINAPSIS_HOME" sinapsis -- sync --config sinapsis.config.json --dry-run`
...
`examples/proba/tools/proba-tools/ADAPTACIONES.md`. `tools build` tiene que salir 0 y cada
ítem `kind: "tool"` del rail tiene que abrir una vista registrada. Si no hay herramientas,
omite este paso y no inventes ninguna.

## Paso 5 — Publicar y verificar

1. `set -a; source "$SINAPSIS_HOME/apps/api/.env"; set +a` y luego
   `pnpm --dir "$SINAPSIS_HOME" sinapsis -- sync --config sinapsis.config.json --tools`
   (el `--tools` solo si hay bundles). Anota el conteo de páginas creadas/actualizadas y
   TODOS los avisos; un aviso que no entiendas se investiga, no se ignora.
2. `pnpm --dir "$SINAPSIS_HOME" sinapsis -- status --config sinapsis.config.json`.
...
5. Materia sincronizada: salida de `sync` y de `status`, y el checklist de calidad con su
   resultado.
