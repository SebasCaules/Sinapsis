...
> `raw/Unidad-01/Practica/ITBA TP 8 Seguridad.pdf` · **3 páginas** · archivado el **15/09 a las
> 23:36**, junto con el deck de la Clase 11 y todo el material NoSQL de la semana siguiente.
> Título literal: *"TP N°8 - Seguridad"* · header `Base de Datos II - TP 8` · footer `ITBA - 2026`.
>
> **No hay deck de práctica** —como en todos los martes desde el 11/08— **y no hay dataset**: ni
> `.sql`, ni CSV. Los tres ejercicios traen su propio esquema: el 1 y el 2 **como imagen**, el 3 como
> una línea de texto *(ver § **Los esquemas que hacen falta**)*.
>
> **Teórica que lo sustenta:** **[[Clase 11 - Seguridad-Transacciones]]** *(lunes 07/09)* — y, de
> sus 38 slides, **solo los slides 8 a 13** *(usuarios, `GRANT`, roles, `REVOKE`, ejemplos)*: el TP no
> toca transacciones, ni concurrencia, ni niveles de aislamiento, ni los índices con los que cierra
> el deck.
>
> Calendario: [[_cronograma]] *(fila 08/09 — tema oficial: **"TP 8 - Seguridad"**)* ·
> Índice: [[_index-clases]] · TPs: `raw/tp/_index.md`

> [!important] 🎯 El TP es de **grafos de permisos**, y el deck 11 **no tiene ninguno**
> El deck enseña la **sintaxis** de MySQL —`CREATE USER`, `GRANT … WITH GRANT OPTION`, `CREATE ROLE`,
> `REVOKE`, `FLUSH PRIVILEGES`— en seis slides. El TP pide otra cosa: **razonar la propagación** de
> privilegios entre usuarios —quién puede re-otorgar qué, qué se cae cuando el otorgante pierde el
> privilegio— y el ejercicio 1 lo dice con el nombre técnico: *"Realice el **grafo de permisos**"*.
>
> Ese formalismo **no está en el deck**: está en la bibliografía obligatoria, **GMUW cap. 10.1.5
> *Grant Diagrams* (impresa 431) y 10.1.6 *Revoking Privileges* (433)**. Para resolver el TP hacen
> falta las dos cosas: la sintaxis del deck y el modelo del libro.

> [!warning] 🔴 El enunciado nombra a **MySQL** tres veces — dos de ellas **para decir que no alcanza**
...
>   teoría**"*
>
> Es el **tercer** TP consecutivo que separa lo que pide el estándar de lo que soporta MySQL
> *(TP6 3.b/3.c: "en SQL estándar" vs. "soportadas por MySQL"; TP7 1.c/2.b: "según la teoría"; y
> ahora TP8 1.b y 2)*. Pero aquí el desfasaje es más profundo que una cláusula que falta:
> **el modelo de seguridad de MySQL no tiene *owner*, no tiene `PUBLIC`, no tiene `CASCADE` y
> maneja el `GRANT OPTION` como una marca por tabla, no por privilegio.** Cuatro diferencias, y las
> cuatro cambian la respuesta de algún ítem → § *Qué de este TP no se puede correr en MySQL*.

> [!note] El PDF: 3 páginas, **2 imágenes**, y una de ellas es un esquema que el vault ya conoce
> - **Página 1**: ejercicio 1 entero, con un ERD pequeño *(2 tablas)* como imagen de 344×144 px.
> - **Página 2**: ejercicio 2 entero, con el **ERD de Voluntarios** como imagen de 745×456 px. **Es el
>   mismo esquema del slide 5 de [[Clase 05 - Consultas de Datos–Parte 1]]** —las 7 tablas, con tipos
>   Oracle (`NUMBER`, `VARCHAR2`)—; la transcripción completa vive ahí.
> - **Página 3**: ejercicio 3, texto puro, y la página queda vacía de la mitad para abajo.
>
> El texto extraído **no trae ninguno de los dos esquemas**: los tipos y las nulabilidades de abajo
> salen de mirar los PNG renderizados a 250 dpi.
...
| :---: | --- | --- | --- | --- | :---: |
| **1.a** | grafo de permisos de 6 `GRANT` en cadena, y detectar cuál falla | `ALUMNO`, `PARRAFO` *(owner `db_exp`)* | `db_exp` → `adm` → `doc` | `WITH GRANT OPTION`, privilegio **por columna** | 🟡 corre, **pero con otro resultado** que la teoría |
...
| **2.a–f** | 6 sentencias de control de acceso sobre Voluntarios | **Voluntarios** *(el de la Clase 05)* | `U0` admin, `U1`, `U2`, `U3` | `ALL`, `GRANT OPTION`, **`PUBLIC`**, `REVOKE` | 🟡 todo menos `PUBLIC` (d, f) |
...
| **3.a–c** | quién puede qué, y qué queda tras un `REVOKE … CASCADE` | `USUARIO(nro_u, nombre, tarea)` *(creada por `A`)* | `A` → `B`, `C`, `D` | grafo de nuevo, más pequeño; **notación `A.usuario`** | 🟡 a y b.1 sí; **b.2 no** (`CASCADE`) |

> [!success] 🎯 Cierra una previsión de [[MySQL]] § *Qué TPs corren sobre MySQL*: *"TP8 — Seguridad — ✅ roles y permisos"*
> La tabla *"Qué TPs corren sobre MySQL"* de [[MySQL]] tenía al TP8 como **✅ roles y permisos**.
> **Acertó en el tema y a medias en el motor**: de los **18 ítems** *(1.a, 1.b, 2.a–j, 3.a.1–3, 3.b.1–2,
> 3.c)*, **tres se resuelven solo en papel** por decisión del enunciado *(1.b y los dos `PUBLIC` de
> 2.d/2.f)*, **uno no puede ejecutarse** porque nombra un rol que no existe *(2.h)*, y **uno usa
> `CASCADE`** *(3.b.2)*. El resto corre — con las salvedades del § *Qué de este TP no se puede correr
> en MySQL*. Y **no reutiliza `esq_peliculas.sql`**: el TP7 sí lo había retomado *(ej. 1 y 3)*; el TP8
> vuelve a traer esquemas propios en los tres ejercicios.
...
| **Revocación parcial de un privilegio de tabla** | `REVOKE UPDATE(tiempo)` a quien tiene `UPDATE` sobre **toda** la tabla. Es el punto más fino del TP y **no está ni en el deck ni en el enunciado explicado** | ej. 1.b |
...
> [!success] 🎯 Tercer TP consecutivo que separa el estándar de MySQL — ya es el método de la cátedra, no una anécdota
> TP6 *(3.b/3.c)*: *"en SQL estándar"* vs. *"las que puedan ser soportadas por MySQL"*. TP7: *"aunque MySQL
> no soporta esta última opción, resuelvalo según la teoría"* *(1.c)* y *"aunque MySQL no lo soporta,
> responda según la teoría"* *(2.b)*. **TP8 *(1.b, 2)*: *"Resuelva el ejercicio
> desde la teoría, ya que MySQL no provee la opción CASCADE"***.
>
> La idea es siempre la misma y la respuesta también: **dos párrafos rotulados**, uno con lo que
> dice el estándar y otro con lo que hace MySQL. En este TP, además, el enunciado **anticipa la
> brecha antes de que aparezca** *(el ej. 2 la avisa antes de listar los ítems)*. Lo que sigue sin
> decir es **qué se rinde en el parcial** → [[PostgreSQL]] § *Dudas abiertas*.

---

## Los esquemas que hacen falta, y de dónde salen

**El TP8 no trae `.sql`.** Los tres esquemas hay que crearlos a mano para poder ejecutar algo. Lo que
sigue es **DDL propio, en MySQL**, transcripto de las imágenes del PDF; los tipos Oracle del ERD de
Voluntarios se traducen como se hizo en los TPs anteriores.

### 1 · Esquema del experimento lingüístico — ejercicio 1

Imagen de la página 1 *(344×144 px)*. Dos cajas con notación de Erwin/Power Designer: el compartimento
superior es la PK, la llave en rojo marca la FK.

| Tabla | Columna | Rol en el diagrama |
| --- | --- | --- |
| **ALUMNO** | `id_alumno` | **PK** |
| | `nombre_apellido` | |
| | `pais` | |
| | `fecha_nacimiento` | |
| | `sexo` | |
| **PARRAFO** | `id_alumno` | **PK** (parte 1) + **FK** → `ALUMNO` *(el diagrama lo rotula `(FK)`)* |
| | `nro_parrafo` | **PK** (parte 2) |
| | `tiempo` | *el tiempo de lectura que mide el experimento* |
| | `diccion` | |

> [!note] El diagrama **no trae tipos ni nulabilidades** — los de abajo son elección propia
> A diferencia del ERD de Voluntarios de la página 2, este solo muestra nombres. Para el ejercicio da
> lo mismo: **ninguna sentencia del ej. 1 depende de un tipo**. Solo importa que `tiempo` y `diccion`
> sean columnas de `PARRAFO`, porque son las que recibe el `GRANT UPDATE(tiempo, diccion)`.
...
del slide 5 de [[Clase 05 - Consultas de Datos–Parte 1]]** *(el esquema HR de Oracle renombrado, como
esa página ya documenta)*. Transcripción de lo que se lee a 250 dpi:
...
> [!note] `ciudad` y `provincia` dicen **`VARCHAR`**, no `VARCHAR2` — y la página de la Clase 05 las transcribió como `VARCHAR2`
> A 250 dpi se lee sin ambigüedad: `ciudad: VARCHAR(30) NOT NULL` y `provincia: VARCHAR(25) NULL`. Es
> una inconsistencia **del diagrama original** *(mezcla `VARCHAR2` de Oracle con `VARCHAR` estándar en
> la misma caja)*, no del TP. Para el TP8 no cambia nada. Queda anotado para
> [[Clase 05 - Consultas de Datos–Parte 1]] § *Slide 5*, que habría que revisar contra su propio PNG.

El TP solo toca **tres tablas**: `INSTITUCION` *(2.a, 2.e)*, `VOLUNTARIO` *(2.b, 2.c, 2.g, 2.i)* y
`TAREA` *(2.d, 2.f)*. Con esas tres alcanza para ejecutar todos los ítems, pero como `VOLUNTARIO`
referencia a `INSTITUCION` y a `TAREA`, se crean en ese orden:
...
> [!tip] `INSTITUCION` ↔ `VOLUNTARIO` se referencian mutuamente — el mismo ciclo que `esq_peliculas.sql` resuelve con un `DROP FOREIGN KEY`
> `institucion.id_director` → `voluntario` y `voluntario.id_institucion` → `institucion`. Por eso la
> FK del director se agrega **después** de crear las dos tablas. Para este TP no hace falta cargar
> datos: **los `GRANT` se pueden ejecutar sobre tablas vacías** y se verifican con `SHOW GRANTS`.

### 3 · `USUARIO` — ejercicio 3

Una sola línea de texto: *"El usuario A ha creado la tabla USUARIO(nro_u, nombre, tarea)"*. Sin tipos.
El `INSERT` del punto a.2 pone `'C'` en `nro_u`, así que **`nro_u` es texto**, no número, pese al
prefijo `nro_`.
...
El motor es el mismo contenedor de toda la cursada — `mysql:9.7.2`, usuario `root`/`root` — descrito
en [[MySQL]] § *Setup, tal como lo da la práctica*. Lo que **cambia** respecto de los TPs anteriores es
que aquí hace falta **más de una sesión**: para comprobar que `doc` no puede hacer algo hay que
conectarse **como `doc`**.
...
**Todos los usuarios que nombra el TP hay que crearlos antes**, porque desde MySQL 8.0 un `GRANT` a
un usuario inexistente **no lo crea: falla** *(ver § *Qué no corre*, fila 1)*. Los tres ejercicios
...
>    *(las dos sentencias, a continuación)*.
...
| **`WITH GRANT OPTION`** | Se adjunta **a cada privilegio**: se puede tener `SELECT*` y `DELETE` sin `*` sobre la misma tabla | 🔴 Es **un único privilegio `GRANT OPTION` por nivel** *(global, base, tabla)*: si se tiene sobre la tabla, sirve para re-otorgar **cualquier** privilegio que se tenga sobre ella |
...
| **Roles** | SQL:1999: `CREATE ROLE`, `GRANT rol TO usuario`, `GRANT priv TO rol` | ✅ Desde 8.0 *(slide 11)*. 🔴 **Un rol otorgado no está activo hasta `SET DEFAULT ROLE` o `SET ROLE`** |
...
| **`FLUSH PRIVILEGES`** | — | El slide 12 dice que *"se deben refrescar"* tras cada `GRANT`/`REVOKE`. 🔶 **Solo hace falta si se editaron las tablas `mysql.*` a mano**; `GRANT` y `REVOKE` recargan solos → § *Contradicciones* |
...
y **10.1.6** *Revoking Privileges* (433) — es **la** fuente del ejercicio 1. **Date 8ª ed. cap. 17.2**
...
> [!important] 🎯 La sentencia 6 es **la** respuesta del punto a: *"si alguna de ellas arroja algún error"*
> El enunciado construye la trampa con cuidado: las dos primeras concesiones a `adm` llevan `WITH GRANT
> OPTION` y la tercera **no**. Después `adm` re-otorga las tres. **Las dos primeras pasan, la tercera
> falla.** Y la falla es silenciosa en el sentido de que `adm` **sí puede borrar** de `PARRAFO` — lo
> que no puede es **autorizar a otro** a hacerlo. Es la distinción entre *tener* y *poder ceder* que
> el slide 10 comprime en una línea.
...
>   separados, porque cada uno se revoca por separado. Dibujar "adm" como un solo nodo con tres
>   etiquetas oculta exactamente lo que el punto b pregunta.
...
>   conviene dibujar, porque son los que sostienen a `doc·UPDATE(tiempo,diccion)*` y los que corta
>   el segundo `REVOKE` del punto b.

### 1.a · Y en MySQL, la sentencia 6 **no falla**

> [!bug] 🔴 En MySQL las seis sentencias pasan — y el grafo queda **distinto** del de la teoría
...
> ese nivel* **y** *tener el privilegio que se otorga*.
>
> Entonces: la sentencia 1 le da a `adm` la marca `Grant` sobre `parrafo`; la 3 le da `Delete`. Al
> llegar a la 6, `adm` **tiene las dos cosas que MySQL pide** → `doc` recibe `DELETE`. **El `WITH GRANT
> OPTION` que faltaba en la sentencia 3 no restringe nada, porque la marca ya estaba.**
>
> El manual lo advierte con una frase que vale la pena citar en la defensa: hay que tener cuidado a
> quién se le da `GRANT OPTION`, porque **dos usuarios con privilegios distintos pueden combinarlos**.
> *(Razonamiento propio sobre la estructura de `tables_priv`; el número de error y la frase exacta
> del manual 9.7 quedan **pendiente de verificar** → § Dudas abiertas.)*

Y hay una **segunda** consecuencia, menos visible: la sentencia 5 le pone a `doc` la marca `Grant`
sobre `parrafo`. A partir de ahí `doc` puede re-otorgar **`SELECT`** —que recibió **sin** grant option
en la sentencia 4— porque MySQL no distingue con qué privilegio vino la marca. Con `mysql.tables_priv`
se ve directo:
...
implica. Revocarle una columna a quien tiene la tabla entera se lee así en el modelo del estándar:
**el privilegio de tabla equivale al conjunto de privilegios de columna**, y se retira uno del
conjunto.
...
option.** Nada más: `SELECT` cayó en cascada, `UPDATE(tiempo)` cayó en cascada, y `DELETE` nunca lo
tuvo *(sentencia 6 fallida)*.
...
> [!warning] ⚠️ Hay una **segunda lectura** del segundo `REVOKE`, y conviene nombrarla al entregar
> Si se toma el privilegio de tabla como **atómico** —`adm` tiene *"UPDATE"*, no *"UPDATE sobre cada
> columna"*—, entonces `REVOKE UPDATE(tiempo) FROM adm` **no encuentra qué revocar**: `adm` nunca
> recibió un privilegio de columna. Con esa lectura la sentencia es un error, nada cambia y `doc`
> conserva `UPDATE(tiempo, diccion) *`.
>
> **Esa es, de hecho, la lectura de MySQL** *(ver abajo)*. La del estándar es la de la descomposición.
> Como el enunciado dice explícitamente *"desde la teoría"*, la respuesta principal es la primera —
> **pero cuál de las dos espera la cátedra es una pregunta para el docente**. GMUW 10.1.6 es donde hay
> que verificar cómo trata el libro la revocación parcial → § *Dudas abiertas*.

> [!tip] Cómo cerrar el punto b al entregar
> Enumerar **los tres estados de `doc`** en una tabla: *antes*, *tras el 1er REVOKE*, *tras el 2do*.
> Y decir en voz alta la regla: *"un privilegio sobrevive a un `CASCADE` solo si le queda otro camino
> hasta el owner"*. Aquí `doc` tenía **un solo camino para todo** —siempre vía `adm`—, así que todo lo
> que `adm` pierde, `doc` lo pierde. Si `db_exp` le hubiera otorgado `SELECT` a `doc` **directamente**,
> ese `SELECT` habría sobrevivido: es el caso que el ejercicio 3 explora.

### 1.b · Y en MySQL

| Sentencia | En MySQL | Efecto sobre `doc` |
| --- | --- | --- |
| `REVOKE SELECT ON parrafo FROM adm CASCADE` | 🔴 **`CASCADE` es error de sintaxis** (`ERROR 1064`). Sin la palabra, `REVOKE SELECT ON experimento.parrafo FROM 'adm'@'%'` **sí corre** | **Ninguno.** `adm` pierde `SELECT`; **`doc` lo conserva**, porque MySQL no registra *quién* otorgó qué y no propaga nada |
| `REVOKE UPDATE(tiempo) ON parrafo FROM adm CASCADE` | 🔴 Ídem `CASCADE`. Y sin él, **también falla**: `adm` tiene `UPDATE` de **tabla**, no de columna; MySQL no descompone → `ERROR 1147 There is no such grant defined for user 'adm' on host '%' on table 'parrafo'` *(número **pendiente de verificar**)* | **Ninguno** |

**En MySQL, `doc` conserva todo lo que tenía: `SELECT`, `UPDATE(tiempo, diccion)`, `DELETE` y la
marca `Grant`.** Es exactamente la razón por la que el enunciado manda resolverlo *"desde la teoría"*:
en el motor de la cursada **el ejercicio no tiene contenido**.

> [!important] 🎯 En MySQL, quitarle algo a `adm` **no le quita nada a `doc`**. Eso es lo que hay que decir
> No es un detalle de sintaxis: es un **modelo de seguridad distinto**. En el estándar el grafo tiene
> memoria *(cada privilegio sabe de dónde vino)*; en MySQL las tablas `mysql.*` guardan **el estado
> final** y nada más. Para lograr en MySQL el efecto de un `CASCADE` hay que revocar **a mano** a cada
> beneficiario — y para saber quiénes son, hay que haberlo anotado afuera del motor.
...
>    algún ítem no pueda. Hay al menos uno claro *(h)* y dos por carencia del motor *(d, f)*.
> 3. **U0 "con privilegios de administrador"**: en MySQL es `root`, o una cuenta con
>    `ALL PRIVILEGES ON *.* … WITH GRANT OPTION`. Se asume que U1, U2 y U3 **ya existen** *(creados en
>    § Setup)*; el único que el TP crea explícitamente es **U4**, en h.
...
✅ Corre. Dos precisiones:

- **`ALL` a nivel de tabla** se expande a los privilegios **de tabla** que existen en MySQL
  *(`SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES, INDEX, ALTER, CREATE VIEW, SHOW VIEW,
  TRIGGER`)*. **No** incluye `GRANT OPTION`: por eso el *"con posibilidad de concederlos a otros"*
  necesita la cláusula explícita. El slide 10 lo lista como *"ALL : esta opción otorga todos los
  permisos"* *[sic, con el espacio]* y a `WITH GRANT OPTION` como una opción aparte — coincide.
- `SHOW GRANTS FOR 'U1'@'%'` lo muestra como
  `GRANT ALL PRIVILEGES ON voluntarios.institucion TO 'U1'@'%' WITH GRANT OPTION` *(MySQL lo
  imprime con los identificadores entre acentos graves)*.
...
Para que U2 **autorice** un `INSERT`, U2 tiene que **tener** `INSERT` **y** poder cederlo. U2 solo tiene
`SELECT` *(b)*. Entonces son **dos pasos, ejecutados por dos usuarios distintos**:
...
> aunque U0 nunca quiso eso. En el modelo de la teoría, U2 tendría `INSERT*` y `SELECT` *(sin `*`)*, y el
> `GRANT SELECT` fallaría. **Si lo que se quiere es lo que pide el ítem y nada más, en MySQL no se
> puede expresar.**
...
- **Con** la cláusula: **cada usuario del sistema puede a su vez otorgar `INSERT`/`UPDATE` sobre
  `TAREA` a otros**. Para los usuarios de hoy no cambia nada *(ya lo tienen todos)*; lo que cambia es
  **el grafo**: aparecen aristas usuario → usuario, y por lo tanto **caminos alternativos** al de
  `PUBLIC`. Eso es lo que **complica el ítem f**: revocar de `PUBLIC` con `CASCADE` arrastra esas
  aristas *(todas descienden de la de `PUBLIC`)*, pero **un `REVOKE … RESTRICT` fallaría** por tener
  dependientes. Además, con `GRANT OPTION` **cualquiera puede darle `INSERT` a un usuario que se cree
  mañana** aunque U0 haya cambiado de idea — se pierde el control centralizado.
...
`ALL PRIVILEGES` a la lista explícita sin `DELETE`.

> [!note] Si U1 ya había cedido `DELETE` a alguien *(a lo dio con grant option)*, ese alguien **lo conserva** en MySQL
> En la teoría, el `REVOKE` tendría que llevar `CASCADE` *(y arrastrar a los beneficiarios)* o
> `RESTRICT` *(y fallar)*. En MySQL no hay ni una cosa ni la otra: la sentencia pasa y los beneficiarios
> de U1 quedan intactos. Es el mismo punto del ej. 1.b, ahora sin que el enunciado lo avise.
...
2. **Cualquiera que tenga `INSERT` sobre `TAREA` por un camino que no dependa de `PUBLIC`** — por
   ...
   **no hay ninguna**: U1 tiene `ALL` sobre `INSTITUCION`, no sobre `TAREA`; U2 y U3 tienen cosas
   sobre `VOLUNTARIO`.
3. Y aquí vuelve la pregunta de d: si el d se hizo **con `GRANT OPTION`** y algún usuario re-otorgó
   `INSERT` a otro, en la teoría **con `CASCADE` esas aristas caen también** *(descienden de `PUBLIC`)*,
   y la respuesta sigue siendo *"solo U0"*. **En MySQL, esas re-concesiones sobreviven** — y U0 no
   tiene cómo saber que existen salvo leyendo `mysql.tables_priv`.
...
> [!note] El rol se llama `ins_vol` y **no inserta nada**: actualiza
> El prefijo `ins_` sugiere *insert*, pero la capacidad pedida es `UPDATE` sobre una columna. No es
> un error del ejercicio —el nombre es libre—, pero es un buen momento para notar que **el nombre del
> rol no restringe nada**: lo que puede hacer un rol es exactamente lo que se le otorgó.
...
🔴 **El rol `ins_prov` no existe.** El ítem g creó **`ins_vol`**; ningún ítem anterior creó `ins_prov`.
...
que algún ítem no pueda ser ejecutado"*.**

Dos formas de seguir, y hay que **declarar cuál se toma**:
...
> [!warning] 🔴 El slide 11 muestra `GRANT 'app_developer' TO 'dev1'@'localhost'` y **nada de activación**
> Quien copie el deck tal cual y pruebe como U4 va a encontrar que **no puede actualizar nada**, sin
> ningún error en el `GRANT`. Es el punto de MySQL más fácil de pasar por alto del TP, y el deck no
> ayuda. → § *Contradicciones*.
...
✅ Corre — **si `ins_prov` existe** *(es decir, tras haber resuelto h con la lectura A o la B)*. Los
privilegios de columna **se acumulan**: con la lectura A el rol queda con
`UPDATE (horas_aportadas, nombre)`; con la B, solo `UPDATE (nombre)`. U3 y U4 lo ven **sin ninguna
sentencia adicional**: lo que se otorga al rol lo tiene todo el que lo tiene activo.
...
todos sus miembros a la vez — que es exactamente lo que lo hace útil frente a repartir privilegios
cuenta por cuenta, y exactamente lo que lo hace peligroso.

> [!note] Sesiones abiertas: **pendiente de verificar**
> Si U4 tiene una sesión abierta con el rol activo en el momento del `DROP ROLE`, ¿pierde los
> privilegios en el acto o al reconectarse? El manual 9.7 tiene la respuesta; no la verifiqué →
> § *Dudas abiertas*.
...
`INSERT`, **y D lo conserva**. Además B **conserva la marca `Grant`** *(sigue teniendo `SELECT*`)* —
que en MySQL es lo mismo que decir que puede seguir re-otorgando cualquier cosa que tenga.
...
| **2** | **`REVOKE … CASCADE`** | 1.b · 3.b.2 | 🔴 Error de sintaxis. **El enunciado lo dice** en 1.b. Y sin cascada, **las revocaciones nunca se propagan**: quitarle algo al otorgante no le quita nada al beneficiario |
| **3** | **`GRANT OPTION` por privilegio** | 1.a · 2.c · 2.d · 3.b.2 | 🔴 En MySQL es **una marca por (cuenta, nivel)**. Consecuencias: la sentencia 6 del ej. 1 **pasa**; `doc` y U2 pueden re-otorgar privilegios que recibieron sin grant option; B conserva la marca tras perder `INSERT` |
...
> [!important] 🎯 De los diez, **los ítems 3 y 4 son los que cambian respuestas**, no solo sintaxis
> Sin `CASCADE` *(2)* el ejercicio 1.b y el 3.b.2 se contestan en papel, y el enunciado lo asume. Pero
> el `GRANT OPTION` por tabla *(3)* hace que **la sentencia 6 del ej. 1 pase en MySQL y falle en la
> teoría** — el punto a **tiene dos respuestas correctas según el motor**. Y la ausencia de owner *(4)*
> hace que la raíz del grafo, el nodo `**`, **no exista en el motor de la cursada**: en MySQL todo
> privilegio viene de root o de alguien que lo recibió de root. Conviene contestar el TP **en la
> teoría** y anotar, ítem por ítem, dónde MySQL diverge — que es lo que hace esta página.
...
| :---: | --- | --- |
| **1** | **Ningún esquema viene como `.sql`**: dos son imagen y uno es una línea de texto | DDL propio en § *Los esquemas que hacen falta*. Para los `GRANT` **no hacen falta datos**: tablas vacías alcanzan |
| **2** | **Diez cuentas que el TP da por existentes** *(de once nombradas: `U4` la crea el 2.h)* y MySQL exige crear | Bloque de `CREATE USER` en § *Setup*. Un `GRANT` a una cuenta inexistente falla con `ERROR 1410` |
| **3** | **Probar "quién puede" requiere varias sesiones** | `docker exec -it MyMySql mysql -u doc -p`, una por usuario. `SHOW GRANTS FOR` desde root evita abrirlas, pero no muestra si el rol está activo |
| **4** | **El rol se otorga y "no hace nada"** | `SET DEFAULT ROLE … TO …` o `activate_all_roles_on_login`. `SELECT CURRENT_ROLE()` desde la sesión del usuario para confirmarlo |
| **5** | **`ins_prov` no existe** en 2.h | Es del enunciado, no del motor: hay que **elegir una lectura y declararla** *(A: errata de `ins_vol`; B: rol nuevo vacío)* |
| **6** | **Las tablas `mysql.tables_priv` / `columns_priv` no aparecen en ningún deck**, y son lo único que muestra la marca `Grant` | Consultarlas como root. Es la forma más rápida de ver por qué la teoría y MySQL divergen en el ej. 1 |
| **7** | **`VARCHAR` vs. `VARCHAR2` en el ERD de Voluntarios** | Irrelevante para el TP; anotado para la página de la Clase 05 |

## Trampas del enunciado

- [ ] 🔴 **Ej. 1.a — la sentencia 6 falla en la teoría y pasa en MySQL.** Quien lo pruebe en el
      contenedor va a ver seis `Query OK` y concluir que no hay error. **El enunciado espera el error**:
      `adm` recibió `DELETE` sin `WITH GRANT OPTION`.
- [ ] 🔴 **Ej. 1.b — `REVOKE UPDATE(tiempo)` a quien tiene `UPDATE` de tabla** admite dos lecturas
      *(descomposición vs. atómico)*. Hay que elegir una y decirlo. Esta página responde con la
      descomposición del estándar: `doc` conserva **solo `UPDATE(diccion)` con grant option**.
- [ ] ⚠️ **Ej. 1.b — "los permisos que conserva doc"** presupone la respuesta de 1.a: si se aceptó que
      la sentencia 6 pasó *(lectura MySQL)*, `doc` conserva también `DELETE`. Las dos respuestas
      tienen que ser **consistentes entre sí**.
- [ ] 🔴 **Ej. 2.h — `ins_prov` ≠ `ins_vol`.** Es el ítem que *"no puede ser ejecutado"*. Los ítems i y j
      **dependen de cómo se resuelva**.
- [ ] 🔴 **Ej. 2.d/2.f — `PUBLIC` no existe en MySQL** y el deck tampoco lo nombra. Hay que ir a la
      teoría *(GMUW 10.1.4)* para la respuesta y a `mandatory_roles` para el sustituto.
- [ ] ⚠️ **Ej. 2.c — "permitir que U2 autorice"** son **dos sentencias de dos usuarios**, no una. La de
      U0 es el `WITH GRANT OPTION`; la de U2 es el `GRANT` a U3. Escribir solo la segunda es no haber
      resuelto el ítem.
- [ ] ⚠️ **Ej. 2.g — `ins_vol` actualiza, no inserta.** El nombre despista; la consigna es clara.
- [ ] ⚠️ **Ej. 3 — `A.usuario`** es notación de esquema-dueño. En MySQL hay que crear una base `A`.
- [ ] ⚠️ **Ej. 3.c — ¿se ejecutó el `GRANT` de a.3?** El punto a es hipotético. Declarar el supuesto
      *(aquí: no se ejecutó)*.
- [ ] ⚠️ **Ej. 3.a.2 — `‘Control’`** con comilla tipográfica: no parsea.
...
| --- | --- |
| *"TP8 — Seguridad — ✅ roles y permisos"* → [[MySQL]] § *Qué TPs corren sobre MySQL* | ✅ **Confirmado en el tema, matizado en el motor**: 3 ítems en papel por el enunciado *(1.b, 2.d, 2.f)*, 1 inejecutable *(2.h)*, 1 con `CASCADE` *(3.b.2)*; el resto corre, pero **1.a da otro resultado en MySQL** |
| *"¿Reutiliza `esq_peliculas.sql`?"* → `raw/tp/_index.md` | ✅ **No.** El TP7 lo había retomado *(ej. 1 y 3)*; el TP8 trae tres esquemas propios, uno de ellos **el de Voluntarios de la Clase 05** |
| *"¿La cátedra sabe que el material está escrito contra otro motor?"* → [[Práctica 2026-08-25]] | ✅ **Tercera confirmación escrita**: *"ya que MySQL no provee la opción CASCADE"* y *"En el caso de que MySQL no provee la funcionalidad pedida, resuelvalo desde la teoría"* |
| *"¿El parcial se rinde sobre MySQL?"* → [[PostgreSQL]] § *Dudas abiertas* | 🔸 **Sigue abierta.** Y este TP la agrava: en seguridad, **la teoría y MySQL dan respuestas distintas** a la misma pregunta *(ej. 1.a)* |
| *"El deck 11 no dice qué pasa con los usuarios al hacer `DROP ROLE`"* | ✅ Lo contesta el ítem 2.j: se revoca de todos, los usuarios siguen existiendo con sus privilegios directos |

---

## Dudas abiertas

- [ ] 🔴 **Ej. 1.a: ¿la cátedra espera la respuesta de la teoría *(sentencia 6 falla)* o la de MySQL
      *(pasa)*?** El enunciado dice *"si alguna de ellas arroja algún error"*, lo que sugiere la
      primera — pero el ítem no lleva la advertencia *"desde la teoría"* que sí lleva 1.b.
- [ ] 🔴 **Ej. 1.b: ¿revocar `UPDATE(tiempo)` a quien tiene `UPDATE` de tabla se descompone o falla?**
      Esta página responde con la descomposición del estándar. **Verificar en GMUW 10.1.6** cómo lo
      trata el libro con su grafo *(¿tiene aristas de "privilegio general implica el específico"?)*
      antes de la defensa.
- [ ] 🔴 **Ej. 2.h: ¿`ins_prov` es errata de `ins_vol` o un rol distinto a propósito?** Cambia i y j.
- [ ] **Ej. 2.d: ¿la cátedra acepta `mandatory_roles` como sustituto de `PUBLIC`, o espera solo la
      respuesta teórica?** El enunciado dice *"resuelvalo desde la teoría"* cuando MySQL no lo provee;
      el sustituto es un extra.
- [ ] **Ej. 3: ¿`A.usuario` debe reproducirse en MySQL como base de datos `A`?** O se contesta en
      papel, como el ej. 1.b.
- [ ] **¿El `GRANT OPTION` por tabla de MySQL es tal cual se describe aquí?** Razonamiento sobre
      `mysql.tables_priv.Table_priv` *(SET con `'Grant'`)*. **Pendiente de verificar contra el manual
      9.7** § *Privileges Provided by MySQL* y § *GRANT Statement*, incluida la frase sobre *"combinar
      privilegios"*.
- [ ] **Números de error**: `1410` *(GRANT a usuario inexistente)*, `3523` *(rol desconocido)*, `1147`
      *(no such grant)*, `1064` *(`CASCADE`)*. Los tres primeros con confianza alta; **verificar en el
      contenedor 9.7.2**.
- [ ] **`DROP ROLE` con sesiones abiertas**: ¿los privilegios del rol caen en el acto o al
      reconectar? Manual 9.7 § *DROP ROLE Statement*.
- [ ] **¿`mandatory_roles` acepta `SET PERSIST` en 9.7.2 tal como en 8.0?** Y si el rol es obligatorio,
      ¿`REVOKE INSERT … FROM 'todos_tarea'` *(2.f)* funciona igual? Debería: se revoca el privilegio
      del rol, no el rol de las cuentas.
- [ ] **¿El TP8 se entrega?** El 1.b, el 2.d/2.f y el 3.b.2 no producen SQL ejecutable; el resto sí.
      ¿Un `.sql` más un documento con los grafos?

## Preguntas para el docente

1. En el ej. 1.a, ¿la respuesta esperada es la de la teoría *(la sexta sentencia falla)* aunque en
   MySQL no falle? ¿Hay que reportar las dos?
2. En el ej. 1.b, ¿`REVOKE UPDATE(tiempo)` a quien tiene `UPDATE` de toda la tabla se resuelve
   descomponiendo el privilegio *(quedan las otras columnas)* o se considera que no hay nada que
   revocar?
3. ¿`ins_prov` en 2.h es `ins_vol`?
4. Para `PUBLIC`, ¿alcanza con la respuesta del estándar o quieren ver el sustituto de MySQL
   *(`mandatory_roles`)*?
5. ¿El parcial evalúa seguridad con la semántica del estándar *(grafo, `CASCADE`, owner, `PUBLIC`)*
   o con la de MySQL? En este tema **no dan la misma respuesta**.
...
- Conceptos *(propuestos; la etapa de conceptos fija los nombres)*:
...
- Motor: **[[MySQL]]** *(`GRANT OPTION` por nivel, sin `CASCADE`, sin `PUBLIC`, sin owner, roles
  inactivos por defecto — todo de esta página debería propagarse a una sección de seguridad ahí)* ·
  [[PostgreSQL]] *(sí tiene `REVOKE … CASCADE | RESTRICT`, owner y `PUBLIC`: el motor en que el
  enunciado correría tal cual)*
