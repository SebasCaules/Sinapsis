...
| `REVOKE UPDATE(tiempo) ON parrafo FROM adm CASCADE` | (crítico) Ídem `CASCADE`. Y sin él, **también falla**: `adm` tiene `UPDATE` de **tabla**, no de columna; MySQL no descompone → `ERROR 1147 There is no such grant defined for user 'adm' on host '%' on table 'parrafo'` *(✓ verificado en MySQL 9.7.2, 25/09)* | **Ninguno** |
...
> Las sentencias del enunciado, precargadas: alterne la semántica y vea por qué la sentencia 6 falla en la teoría y pasa en MySQL.
...
| :---: | --- | :---: | --- |
| **1** | **`GRANT` a un usuario que no existe** | todos | Desde 8.0 el `GRANT` **no crea la cuenta**: `ERROR 1410 You are not allowed to create a user with GRANT` *(✓ verificado en MySQL 9.7.2, 25/09)*. Hay que ejecutar `CREATE USER` antes *(§ Setup)*. El TP no crea a `db_exp`, `adm`, `doc`, `U1`–`U3`, `A`, `B`, `C` ni `D`; solo a `U4` *(2.h)* |
...
| **6** | **Revocar una columna a quien tiene la tabla entera** | 1.b | MySQL **no descompone** el privilegio de tabla: `REVOKE UPDATE(tiempo)` a quien tiene `UPDATE` de tabla falla *(`ERROR 1147`, ✓ verificado en MySQL 9.7.2, 25/09)* |
...
- [ ] ¿El parcial evalúa seguridad con la semántica del estándar *(grafo, `CASCADE`, owner, `PUBLIC`)* o
  ...
  - (nota) Evidencia de exámenes viejos: la Pregunta 32 de [[Parcial 2Q2025]] § *Sección E* pide una
    cadena de `GRANT`/`REVOKE … CASCADE` resuelta **en SQL estándar**, explícitamente, con el grafo de
    permisos (puntaje completo en la captura para el estado final del solucionario). El handout del
    07/09 de la [[Clase 11 - Seguridad-Transacciones|Clase 11]] también da por buena la semántica del
    estándar. Es un examen de 2025: indicio, no confirmación del parcial del 13/10/2026.
- [ ] **¿El `GRANT OPTION` por tabla de MySQL es tal cual se describe aquí?** Razonamiento sobre
  ...
  - (nota) Una corrida en MySQL 9.7.2 es consistente con la marca por (cuenta, tabla): tras
    `GRANT INSERT ON t1 … WITH GRANT OPTION` y `REVOKE INSERT ON t1` (sin `CASCADE`), `SHOW GRANTS`
    deja `GRANT USAGE ON t1 … WITH GRANT OPTION`: la opción sobrevive sin el privilegio con el que
    vino → [[Clase 11 - Seguridad-Transacciones]] § *Material complementario del 07/09*. La frase del
    manual sigue sin verificar.
- [x] ~~**Números de error**: `1410` *(GRANT a usuario inexistente)*, `3523` *(rol desconocido)*, `1147`
  *(no such grant)*, `1064` *(`CASCADE`)*.~~ (ok) ✓ Los cuatro verificados en MySQL 9.7.2:
  - `1064`: `REVOKE … CASCADE` da `ERROR 1064 (42000): You have an error in your SQL syntax … near
    'CASCADE'` → [[Clase 11 - Seguridad-Transacciones]] § *Material complementario del 07/09*.
  - `1410` (25/09): `GRANT` a una cuenta inexistente da `ERROR 1410 (42000): You are not allowed to
    create a user with GRANT`.
  - `1147` (25/09): `REVOKE UPDATE(tiempo)` a quien tiene `UPDATE` de tabla da `ERROR 1147 (42000):
    There is no such grant defined for user … on host '%' on table 'parrafo'`.
  - `3523` (25/09): `GRANT` de un rol inexistente da `ERROR 3523 (HY000): Unknown authorization ID
    `` `ins_prov`@`%` ``, con acentos graves, no comillas simples.
...
- Exámenes viejos: [[Mapa de exámenes]] *(el [[Parcial 2Q2025]] trae un grafo de `GRANT`/`REVOKE …
  CASCADE` del mismo tipo que el ejercicio 1)*
