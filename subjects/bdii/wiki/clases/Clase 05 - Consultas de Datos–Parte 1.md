...
- `IS NULL` / `IS NOT NULL` es la única forma válida de testear un nulo: `= NULL` nunca da error,
  siempre devuelve cero filas.
- Comparar contra `NULL` produce `UNKNOWN`, no `FALSE`; por eso `!=` y `NOT` dejan afuera las filas
  con nulo.
- Si un `WHERE` mezcla `AND` y `OR`, van paréntesis siempre: sin ellos el motor no falla, pero
  devuelve el conjunto equivocado, por la precedencia `NOT` > `AND` > `OR`.
- `DISTINCT` se aplica a la lista completa de columnas, no a una sola, y trata todos los `NULL` como
  un mismo valor.
- Toda columna no agregada del `SELECT` debe estar en el `GROUP BY` —obligatorio también en MySQL
  desde la versión 5.7, por `ONLY_FULL_GROUP_BY`— y un filtro sobre un agregado va siempre en
  `HAVING`, nunca en `WHERE`.
- `ORDER BY a DESC, b` deja `b` en orden ascendente: `DESC` se aplica por columna, no a toda la
  cláusula.

Para el parcial importan sobre todo la distinción `WHERE`/`HAVING`, la lógica trivaluada con nulos y
el orden real de evaluación de las cláusulas (`FROM → WHERE → GROUP BY → HAVING → SELECT →
ORDER BY → LIMIT/OFFSET`), que no coincide con el orden en que se escriben.

## Fuente

> [!info] Fuente
> `raw/Unidad-01/Teorica/BD2_Clase 05 - Consultas de Datos–Parte 1.pdf` · **36 slides**. Dictada el
> **lunes 03/08**, de forma **asincrónica**. El `05` del nombre del archivo es el número de clase
> (`BD2_Clase NN` → Clase NN). La **Clase 05 es una sola clase repartida en tres archivos**, y esta
> página es la **parte 1 de 3**: sigue en [[Clase 05 - Consultas de Datos–Parte 2]] y cierra en
> [[Clase 05 - Consultas de Datos–Parte 3]]. Clase anterior:
> [[Clase 04 - AlteraciónActualizaciónTablas]]. Índice general: [[_index-clases]]. Se practica con
> `ITBA TP 3 SQL simples.pdf` sobre `esq_peliculas.sql` → [[Práctica 2026-08-04]]. Bibliografía:
> [[_index-bibliografia]].

> [!warning] Este deck **no** cubre fechas ni funciones de string
> A pesar de que el cronograma anuncia "Repaso de SQL, SQL avanzado, manejo de fechas", la parte 1 no
> toca fechas más allá de `MAX`/`MIN` sobre `fecha_nacimiento` (slide 25), ni funciones de string, ni
> `JOIN`, subconsultas o `IN` — eso queda para las partes 2 y 3. A cambio, sí llega hasta funciones de
> agregación, `GROUP BY` y `HAVING` (slides 23–35), que uno esperaría en la parte 2.
...
El DDL y el `INSERT`/`UPDATE`/`DELETE` son la [[Clase 04 - AlteraciónActualizaciónTablas]]
(`BD2_Clase 04`); esta clase es solo la última línea del cuadro.
...
- SQL es **declarativo**: se especifica qué información se requiere, no cómo obtenerla ni el método
  de acceso.
- **`SELECT` identifica las columnas a recuperar – EL QUE**; **`FROM` identifica la tabla – DE DONDE**.
- El resultado de una consulta es siempre una **tabla** (un número es una tabla de una fila y una
  columna): es la propiedad de cierre del modelo relacional, y por eso una consulta se puede anidar.

> [!note] Notación del diagrama — razonamiento propio, el deck no la explica
> `|` = alternativa · `{ }` = agrupación · `[ ]` = opcional · `,...` = repetible: o `*`, o una lista de
> columnas/expresiones con alias opcional, con `DISTINCT` opcional adelante de toda la lista.
...
> [!warning] "No son sensibles a mayúsculas/minúsculas" vale solo para las **palabras clave**
> Los identificadores (tablas, columnas) sí dependen del motor y del sistema de archivos, y los datos
> comparados con `=` o `LIKE` dependen de la *collation*. Ver [[#PostgreSQL/Oracle → MySQL]].

## Slide 5 · Esquema de ejemplo — BD Voluntarios

El slide se titula "Esquema Ejemplo DERE BD Voluntarios" y muestra el diagrama con las 7 tablas, sus
tipos y su nulabilidad (los atributos en rojo son las FK; arriba de cada caja va la PK). Transcripción
literal de los tipos:

| Tabla | Columnas — tipo · nulabilidad · rol |
| --- | --- |
| **CONTINENTE** | `id_continente` NUMBER NOT NULL **PK** · `nombre_continente` VARCHAR2(25) NULL |
| **PAIS** | `id_pais` CHAR(2) NOT NULL **PK** · `nombre_pais` VARCHAR2(40) NULL · `id_continente` NUMBER NOT NULL **FK**→CONTINENTE |
| **DIRECCION** | `id_direccion` NUMBER(4) NOT NULL **PK** · `calle` VARCHAR2(40) NULL · `codigo_postal` VARCHAR2(12) NULL · `ciudad` VARCHAR2(30) NOT NULL · `provincia` VARCHAR2(25) NULL · `id_pais` CHAR(2) NOT NULL **FK**→PAIS |
| **INSTITUCION** | `id_institucion` NUMBER(4) NOT NULL **PK** · `nombre_institucion` VARCHAR2(60) NOT NULL · `id_director` NUMBER(6) NULL **FK**→VOLUNTARIO · `id_direccion` NUMBER(4) NULL **FK**→DIRECCION |
| **VOLUNTARIO** | `nro_voluntario` NUMBER(6) NOT NULL **PK** · `nombre` VARCHAR2(20) NULL · `apellido` VARCHAR2(25) NOT NULL · `e_mail` VARCHAR2(25) NOT NULL · `telefono` VARCHAR2(20) NULL · `fecha_nacimiento` DATE NOT NULL · `id_tarea` VARCHAR2(10) NOT NULL **FK**→TAREA · `horas_aportadas` NUMBER(8,2) NULL · `porcentaje` NUMBER(2,2) NULL · `id_institucion` NUMBER(4) NULL **FK**→INSTITUCION · `id_coordinador` NUMBER(6) NULL **FK**→VOLUNTARIO *(recursiva)* |
| **TAREA** | `id_tarea` VARCHAR2(10) NOT NULL **PK** · `nombre_tarea` VARCHAR2(35) NOT NULL · `min_horas` NUMBER(6) NULL · `max_horas` NUMBER(6) NULL |
| **HISTORICO** | `fecha_inicio` DATE NOT NULL **PK** (parte 1) · `nro_voluntario` NUMBER(6) NOT NULL **PK** (parte 2) + **FK**→VOLUNTARIO · `fecha_fin` DATE NOT NULL · `id_tarea` VARCHAR2(10) NOT NULL **FK**→TAREA · `id_institucion` NUMBER(4) NULL **FK**→INSTITUCION |

Tres cosas del esquema que después explican los ejemplos: `id_coordinador` es una FK recursiva a
`VOLUNTARIO` y es `NULL`able (hay voluntarios sin coordinador, de ahí el `IS NULL` del slide 15);
`porcentaje` es `NUMBER(2,2)` —precisión 2, escala 2 en Oracle, valores menores que 1 con dos
decimales, por eso el slide 16 filtra `<= 0.1` y no `<= 10`—; y `HISTORICO` tiene PK compuesta
`(fecha_inicio, nro_voluntario)`.
...
> *locations*, `pais` ← *countries*, `continente` ← *regions*, `historico` ← *job_history* (lo
> confirman nombres como **Steven King** y códigos como `AD_PRES`, `ST_MAN`). **El TP 3 no usa este
> esquema sino `esq_peliculas.sql`.**

> [!bug] El deck le dice "horas_aportadas" a lo que en HR era el sueldo
> Los valores (`24000.00`, `13500.00`, `3300.00`, `2500.00`) son sueldos de HR, no horas.
> `SUM(horas_aportadas)` de todos da `691400.00` (slide 23). No cambia la sintaxis, pero los
> resultados **no tienen sentido semántico**: no interpretar los números.
...
> Razonamiento propio, no está en el deck: (a) transfiere columnas que no se usan, más I/O y red;
> (b) impide que un índice sea *covering*; (c) rompe el código si alguien agrega, saca o reordena una
> columna y el consumidor accede por posición; (d) hace ilegible qué necesita la consulta. Se retoma
> con índices y explain plan en [[Clase 08 - Explicando el plan]].
...
> `27 fila(s)` + `Tiempo total de ejecución: 2.589 ms` es lo que hay que mirar con el
> **explain plan** ([[Clase 08 - Explicando el plan]]). Y `SELECT *` devuelve acá las columnas en un
> orden que no es el del diagrama del slide 5 (`nombre_institucion` primero): otra razón para no usarlo.
...
Es el punto donde SQL se separa del álgebra relacional: una tabla SQL es un *multiconjunto* (bag),
no un conjunto.
...
> "el mismo valor", aunque `NULL = NULL` no sea verdadero — excepción a la regla del slide 12.
> 2. **`DISTINCT` no ordena**: el resultado sale `NULL, 205, 122, 120, …`. Si hace falta orden,
> `ORDER BY` (slide 20).
> 3. La consulta devuelve la columna `id_coordinador` de `voluntario`: son los coordinadores
> referenciados por la FK recursiva. La fila `NULL` significa "hay voluntarios sin coordinador".
...
`WHERE` se usa para realizar restricciones: se recuperan las filas cuyos datos satisfacen **todas**
las condiciones lógicas. Una cláusula `WHERE` contiene condiciones lógicas que utilizan:
...
> `<>` es el estándar SQL; `!=` lo aceptan MySQL, PostgreSQL y Oracle por igual. Para el parcial,
> cualquiera de los dos.

## Slide 12 · Condiciones de comparación y lógica trivaluada

Al utilizar los operadores de comparación el resultado puede ser **Verdadero (T)**, **Falso (F)** o
**Desconocido (U)**. Y a continuación, con la frase "si se comparan valores nulos" subrayada en rojo:
...
> `e-mail` con guión medio: **no compila** — el parser lo lee como la resta `e - mail` y falla con
> "columna `e` inexistente". Los slides 14, 15 y 16 la escriben bien; copiar tal cual los ejemplos de
> 12 y 13 da error.

> [!bug] El slide se contradice a sí mismo, y esto entra en el parcial
> Arriba dice que el resultado puede ser `U`; abajo, que comparar contra nulo da "siempre FALSO". Lo
> correcto es lo primero: da **UNKNOWN**, un tercer valor lógico, no `FALSE`. `WHERE` devuelve la fila
> solo si `TRUE` —descarta `FALSE` y `UNKNOWN` por igual, por eso "parece" falso—, pero se rompe con
> la negación (razonamiento propio):
>
> | Expresión | Si `x IS NULL` | Con la regla del slide ("es FALSE") |
> | --- | --- | --- |
> | `x = 5` | `UNKNOWN` → descartada | `FALSE` → descartada ✓ |
> | `NOT (x = 5)` | `NOT UNKNOWN` = **`UNKNOWN`** → descartada | `NOT FALSE` = `TRUE` → devuelta ✗ |
> | `x != 5` | **`UNKNOWN`** → descartada | `TRUE` → devuelta ✗ |
>
> Consecuencia práctica: las filas con `id_tarea IS NULL` no aparecen en ninguna de las dos consultas
> del slide. `=` y `!=` dejan afuera a los nulos las dos veces — el error clásico del TP.
...
Además de los operadores de comparación está disponible el operador especial `[NOT] BETWEEN`, que
trata los valores de los extremos como **incluidos** en el rango.
...
> El deck dice lo primero, no lo segundo: `BETWEEN 120 AND 100` no devuelve nada, porque se traduce a
> `a >= 120 AND a <= 100`. Siempre el menor primero. Por la misma equivalencia, `BETWEEN` hereda el
> comportamiento con nulos de `>=`/`<=`: si `a IS NULL`, da `UNKNOWN` (razonamiento propio).

## Slide 14 · `[NOT] LIKE`

No siempre se conoce el valor exacto a buscar: se pueden buscar coincidencias con un patrón de
caracteres mediante `LIKE`, también en su forma negativa `NOT LIKE`.
...
> 1. El screenshot no corresponde a la consulta: proyecta tres columnas (`nombre, apellido, e_mail`)
> y el resultado muestra ocho, de un `SELECT *`. Pasa igual en el slide 15.
> 2. Las comillas del patrón son tipográficas (`'_a%n'` curvas en el PDF): copiadas tal cual no
> compilan, hay que reescribirlas como comillas simples rectas.

> [!tip] Trampas de `LIKE` que el deck no menciona
> - `_a%n` no encuentra `Ana`: `_` exige exactamente un carácter antes de la `a`, y `Ana` empieza con
> `A` mayúscula seguida de `n`. `%` matchea vacío, `_` no.
> - Sensibilidad a mayúsculas depende del motor: en **MySQL con la collation por defecto**
> (`utf8mb4_..._ci`, *case-insensitive*) `LIKE 'a%'` matchea también `Ana`; en **PostgreSQL `LIKE` es
> sensible** y hace falta `ILIKE`. En el TP 3 sobre MySQL, contar con la insensibilidad solo si se
> verificó la collation.
...
Si una columna carece de valor se dice que contiene un `NULL`: un valor inaccesible, sin valor,
desconocido o inaplicable. No representa ni un cero ni un espacio en blanco (el cero es un número y
el espacio es un carácter). Solo se testean valores que son nulos.
...
FALSO; si se desea incluir en el resultado los datos con nulos hay que hacerlo explícitamente.

*Ejemplo listar los datos de los voluntarios sea menor o igual que 0,10* (algunos porcentajes pueden
ser nulos, y para incluirlos hay que explicitar `IS NULL`):
...
> porcentaje"*. Y escribe el umbral como `0,10` en el texto pero `0.1` en la consulta: en SQL el
> separador decimal es siempre el punto.

> [!important] Este es el operador que hay que memorizar
> `IS NULL` / `IS NOT NULL` son la única forma de testear un nulo. `= NULL` y `!= NULL` no fallan:
> compilan y devuelven `UNKNOWN` para todas las filas, cero resultados sin ningún mensaje de error.
> Es el bug más silencioso del TP.

## Slides 17–19 · Condiciones compuestas y el problema de los paréntesis

**Slide 17 — los tres operadores lógicos.** Un operador lógico combina los resultados de dos
condiciones en un único resultado, o invierte el resultado de una condición.
...
**Slide 19 — la respuesta.** El slide no reescribe la consulta, muestra directo el resultado. La
condición sin paréntesis sería (reconstrucción propia, no está impresa en el slide):
...
> [!warning] "No es válida" es impreciso, y el tercer óvalo cuenta mal
> Los óvalos rojos marcan `101-102-114`, `145-146-147-148-149` y uno que abarca `199 Grant` y
> `201 Hartstein`; pero `199 Grant` sí corresponde (institución 50, coordinador 124, ya en el resultado
> correcto del slide 18), así que la fila sobrante es solo la 201 —son 9 filas de más, no 10. Y la consulta no es "no válida", es válida y
> está mal: sin paréntesis, SQL aplica la precedencia de operadores —`NOT` > `AND` > `OR`— y la
> condición se agrupa así (razonamiento propio, no está en el deck):
>
> ```sql
> WHERE id_coordinador = 100
...
> ```
>
> O sea: todos los coordinados por el 100 sea cual sea su institución, más los coordinados por el 124
> que además estén en la 50. Por eso las 9 filas de más —101, 102, 114, 145, 146, 147, 148, 149 y
> 201— son todas de `id_coordinador = 100` con `id_institucion` ≠ 50. El motor no da error: ejecuta
> perfecto y devuelve el conjunto equivocado. Regla de oro: si en un `WHERE` conviven `AND` y `OR`,
> paréntesis siempre, aunque sean redundantes.

## Slide 20 · `ORDER BY`

El orden de las filas es indefinido; `ORDER BY` las ordena y debe ir como última cláusula del
`SELECT`. Por defecto el orden es ascendente (`ASC`); `DESC` después del nombre de columna da orden
descendente. Se puede ordenar por más de una columna (incluso todas las de la tabla).
...
> En `ORDER BY apellido DESC, nombre` el `DESC` afecta solo a `apellido`; `nombre` queda en `ASC`. Para
> descendente en las dos: `ORDER BY apellido DESC, nombre DESC`. Es el error más común de la cláusula.
> Y "el orden es indefinido" es literal: sin `ORDER BY` el motor puede cambiarlo entre corridas si
> cambia el plan o los índices, aunque el resultado parezca ordenado.

> [!bug] "última cláusula del SELECT" choca con el slide siguiente
> El slide 20 dice que `ORDER BY` va última; el 21 muestra `LIMIT`/`OFFSET` después. Lo correcto:
> `ORDER BY` es la última de las cláusulas *lógicas*, y `LIMIT`/`OFFSET` van detrás de todas.
...
> Es la primera vez en la Clase 05 que el deck declara el motor. La cursada corre sobre MySQL →
> equivalencias abajo y en [[#PostgreSQL/Oracle → MySQL]].
...
Debe usarse siempre con `ORDER BY`. `LIMIT` limita la cantidad de filas a retornar; `OFFSET`
determina a partir de qué fila del resultado se retorna.
...
> [!tip] Por qué "siempre con `ORDER BY`" (razonamiento propio)
> Sin `ORDER BY` el orden es indefinido (slide 20): "los 10 primeros" no significa nada, el motor
> puede devolver diez filas distintas en cada corrida. `LIMIT` sin `ORDER BY` es no determinístico.

> [!warning] `LIMIT ALL` **no existe en MySQL**
> `LIMIT ALL` es sintaxis de PostgreSQL, "sin tope". En MySQL: `LIMIT 10` es igual en los dos motores;
> `LIMIT n OFFSET m` también, y MySQL además acepta la forma corta `LIMIT m, n` (ahí el primer número
> es el offset); `LIMIT ALL OFFSET 15` no compila en MySQL, que exige un `LIMIT` numérico cuando hay
> `OFFSET` (la documentación recomienda `LIMIT 18446744073709551615 OFFSET 15` para "desde la fila 15
> hasta el final"). `OFFSET 15` salta 15 filas y empieza por la 16ª: el enunciado "a partir del 15TO
> voluntario" es ambiguo respecto de si el 15º entra o no. Verificar en clase.
...
> Los screenshots de los slides 23, 30 y 33 muestran la columna como `coordinador`. En el esquema del
> slide 5 se llama `id_coordinador`, y así la usan todas las consultas del deck: es un renombre del
> screenshot, no una columna distinta.

**Slide 24 — el catálogo completo**, para resumir el resultado de una consulta:
...
> Porque la fecha de nacimiento más grande es la más reciente, y nacer más tarde es ser más joven —
> inversión que se pregunta seguido. La consulta no devuelve al voluntario, devuelve dos fechas; para
> saber quién es hace falta subconsulta o `ORDER BY … LIMIT 1`, nada de eso está en la parte 1.
...
> `COUNT(*)` cuenta filas (una fila enteramente `NULL` igual cuenta). `COUNT(col)` cuenta valores no
> nulos de esa columna. Si `col` no admite nulos los dos dan lo mismo; si admite, `COUNT(col) ≤
> COUNT(*)` siempre, y la diferencia es la cantidad de nulos. El deck no menciona `COUNT(DISTINCT
> col)`, la tercera forma.
...
`COALESCE(columna, valor_reemplazo)` fuerza a las funciones de grupo a incluir valores nulos,
retornando un valor en su ocurrencia (la frase del slide dice "en fuerzan", *sic*, mal redactada).
...
> Razonamiento propio, no está en el deck. `AVG` = suma ÷ cantidad de valores no nulos. De los dos
> resultados se deduce el dato oculto: la suma de `porcentaje` es 7,8 y hay 35 voluntarios con
> porcentaje no nulo (7,8 / 35 = 0,222857…). El `COUNT(*)` de la tabla es 107 (slide 28), y
> 7,8 / 107 = 0,072897…, exactamente el segundo resultado.
>
> `COALESCE` no cambia el numerador, cambia el denominador: convierte 72 nulos en 72 ceros, que sí
> entran en la cuenta. Cuál de los dos promedios es "el correcto" es una decisión semántica, no
> técnica: ¿promedio de los que tienen porcentaje, o promedio sobre todos?

## Slide 28 · Alias de columna y alias de tabla

Tres ejemplos con sus resultados, bajo los rótulos "Alias de columna" y "Alias de tabla".
...
> `horas_trabajadas`): es el *case folding* de PostgreSQL para identificadores sin comillas. En MySQL
> el alias conserva las mayúsculas escritas.

## Slides 29–32 · `GROUP BY`

Si se usa `GROUP BY` en un `SELECT`, se dividen las filas de la tabla consultada en grupos; se
aplican las funciones de la lista `SELECT` a cada grupo y se retorna una única fila por grupo.
`GROUP BY` especifica cómo se deben agrupar las filas seleccionadas (en rojo en el slide).
...
> Le falta el `]` de cierre del `GROUP BY`, y sobre todo omite `HAVING`, que el propio deck introduce
> cuatro slides después. La forma completa está en [[#Orden de evaluación de las cláusulas]].
...
> `NULL` forma su propio grupo (hay un voluntario sin institución), a diferencia del slide 27 donde
> las funciones de agregación ignoran los nulos de la columna agregada — son dos reglas distintas. Y
> los grupos no salen ordenados (`90, NULL, 20, 100, 40, …`), hace falta `ORDER BY`. Suman 107, el
> `COUNT(*)` de la tabla entera del slide 28: `GROUP BY` particiona, no descarta.

**Slide 31 — la regla, en rojo y centrada:** todas las columnas de la lista `SELECT`, excepto las
...
> `SELECT` → `GROUP BY` es obligatorio: toda columna no agregada del `SELECT` va en el `GROUP BY`.
> `GROUP BY` → `SELECT` es opcional: se puede agrupar por algo que no se muestra (slide 32), aunque el
> resultado sea inútil en la práctica —una columna de promedios sin decir a qué institución
> corresponde cada uno—, correcto sintácticamente pero ilegible.
>
> Históricamente MySQL permitía violar la regla y devolvía un valor arbitrario de la columna suelta.
> Desde MySQL 5.7 el modo `ONLY_FULL_GROUP_BY` está activo por defecto y esas consultas dan error,
> igual que en PostgreSQL: la regla del slide 31 se aplica tal cual en la cursada.

## Slides 33–34 · `HAVING`

Se puede anexar `HAVING` para restringir grupos: 1) las filas se agrupan por las columnas
especificadas, 2) se aplica la función de grupo, 3) se muestran los grupos que satisfacen `HAVING`.
Ese orden 1-2-3 es el orden de evaluación y es lo que hay que memorizar.
...
> La consulta dice `AS cantidad_de_voluntarios`, pero el screenshot titula la columna
> `cantidad_voluntarios` (sin el `de_`): es de otra corrida, la del slide 30. Los números son válidos
> igual.

## Slide 35 · Funciones de grupo **no válidas**

No se puede utilizar `WHERE` para restringir grupos: se debe usar `HAVING` (el slide dice "restingir",
*sic*). No se pueden utilizar funciones de grupo en `WHERE`.
...
> `ERROR: aggregates not allowed in WHERE clause at character NNN` es el formato de PostgreSQL; en
> MySQL la misma consulta falla con `ERROR 1111 (HY000): Invalid use of group function`. El texto
> cambia, la regla es la misma en los dos motores. El esquema `unc_esq_voluntario` delata además que
> el deck viene de otra universidad.
...
>
> La razón por la que `WHERE COUNT(*) > 7` es imposible: cuando `WHERE` se evalúa, los grupos todavía
> no existen, así que no hay `COUNT(*)` que calcular. No es una prohibición arbitraria, es
> consecuencia del orden de evaluación. Las dos se pueden usar juntas y hacen cosas distintas:
> `… WHERE id_institucion = 50 GROUP BY id_coordinador HAVING COUNT(*) > 7` = "contando solo a los de
> la institución 50, qué coordinadores tienen más de 7".

## Slide 36 · Para Recordar!!!

Cierre textual del deck, condensado (repite el mapa de arriba): `SELECT` recupera datos de las tablas
cargadas —`SELECT` es EL QUE, `FROM` es DE DONDE—; `WHERE` restringe con operadores de comparación
(paréntesis y sangría para legibilidad); `DISTINCT` elimina repetidos; `ORDER BY` ordena y va última;
las funciones de agregación operan sobre conjuntos de filas dando un resultado por grupo; `GROUP BY`
agrupa (toda columna no agregada del `SELECT` debe estar en el `GROUP BY`); no se pueden usar
funciones de grupo en `WHERE`.
...
Verificar en clase, sobre todo la del alias en el `ORDER BY`: MySQL y PostgreSQL la permiten, pero el
deck nunca la muestra.

Forma completa del `SELECT` con todo lo del deck, en el orden en que se escribe (el slide de origen de
cada elemento está indicado en la sección `## Slide N` correspondiente, arriba):
...
> Ver punto abierto 2 de [[CLAUDE]] y la nota equivalente en [[Clase 04 - AlteraciónActualizaciónTablas]].
...
- [ ] ¿Comparar contra `NULL` da `FALSE` o `UNKNOWN` según la cátedra? (el slide 12 afirma las dos cosas)
- [ ] ¿Por qué se recomienda evitar `SELECT *`? (el slide 6 lo pregunta en rojo y no lo responde)
- [ ] `LIMIT`/`OFFSET` en MySQL: ¿el TP acepta `LIMIT m, n` o exige `LIMIT n OFFSET m`? ¿"a partir del
  15TO voluntario" incluye al 15º?
- [ ] `STDDEV`: ¿poblacional (MySQL) o muestral (PostgreSQL) espera el TP?
...
- [ ] ¿`COUNT(DISTINCT col)` se puede usar en el TP y en el parcial, aunque el deck no lo mencione?
- [ ] ¿Cuál es el orden canónico completo de cláusulas? (el diagrama del slide 29 omite `HAVING`)
- [ ] ¿`IN` y subconsultas entran en la parte 2 o en la 3?
- [ ] Los screenshots de los slides 14 y 15 no corresponden a la consulta que muestran: ¿hay una
  versión corregida del deck?
