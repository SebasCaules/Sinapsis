...
La filmina no da más detalle que "conceptos de teoría de compiladores" y "comandos certificados". Lo que sí se puede afirmar con lo que la propia clase ya construyó: certificar un comando es verificar, para cada camino posible del programa, que no viola ninguna de las dos condiciones de la [[flujo-de-informacion#La definición formal|definición de flujo]] —ni la explícita de un `y := f(x)`, ni la implícita de un `if` o un `while` que filtran sin asignación—. Es exactamente el "analizador que sólo mira asignaciones se pierde el `if` y el `while`" que la [[clase-09-flujo-de-informacion#4. Flujo explícito e implícito|Clase 09]] señala como el problema abierto de esa sección: un mecanismo estático que certifique en serio tiene que rastrear también los flujos implícitos, no sólo las asignaciones. *(Lectura nuestra: la filmina no conecta explícitamente "certificación" con "flujo implícito"; la conexión es nuestra, apoyada en que ambas cosas están a dos filminas de distancia dentro del mismo deck.)*
...
- [[clase-09-flujo-de-informacion#6. Mecanismos de control de flujo|Clase 09 — Flujo de información]] — la sección de la que cuelga esta nota
- [[politicas-de-control-de-flujo|Políticas de control de flujo]] — lo que el mecanismo tiene que hacer cumplir: reflexión, transitividad y la relación de dominancia que las etiquetas implementan
- [[flujo-explicito-e-implicito|Flujo explícito e implícito]] — por qué un mecanismo estático que sólo mira asignaciones no alcanza
- [[problema-del-confinamiento|Problema del confinamiento]] — la sección siguiente: qué pasa cuando ni siquiera hay un programa que certificar, porque el problema ya no es de flujo *dentro* de un proceso
- [[video-11-flujo-de-informacion#Mecanismos estáticos y dinámicos|Video 11 — Flujo de información]] — la misma tabla, con el matiz de que la mitad dinámica es la que el título de YouTube confunde con "control de acceso dinámico"
- [[video-11-flujo-de-informacion#Donde lo técnico se termina|Video 11 — Flujo de información]] — las dos citas de Ramele sobre el límite de lo técnico
- [[video-09-pentesting-metodologia#6. Ejemplo resuelto: ataque externo por ingeniería social|Video 09 — Pentesting: metodología]] — el mismo límite, resuelto como un caso de ataque real por ingeniería social
- [[bibliografia#2. Matt Bishop — Computer Security: Art and Science|Bibliografía]] — Bishop cap. 17, *Information Flow*, mapeado a esta clase
