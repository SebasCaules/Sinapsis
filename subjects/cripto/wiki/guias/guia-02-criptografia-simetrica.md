...
resumen: 'Los ocho ejercicios de la Guía 2 con enunciado y resolución en la misma página: cada resolución va plegada debajo de su enunciado, con el razonamiento completo y no sólo el resultado — secreto perfecto, el juego Eav, modos de encadenamiento y las claves débiles de DES — y contrastada con el PDF de soluciones de la cátedra, que coincide en los ocho.'
...
sources: ["raw/guias/guia2/Guia 2 - Criptografía Simétrica.pdf", "raw/guias/guia2/Guia 2 - Criptografía Simétrica - Soluciones.pdf", "raw/guias/guia2/Resolucion Guia 2.md", "raw/practicas/Clase 3 - Modo CFB.pdf", "raw/practicas/Clase 3.pdf"]
---

# Guía 2 — Criptografía Simétrica

> **24/08/2026**, continúa el **31/08** · [Enunciado](../../raw/guias/guia2/Guia%202%20-%20Criptograf%C3%ADa%20Sim%C3%A9trica.pdf) · [Soluciones de la cátedra](../../raw/guias/guia2/Guia%202%20-%20Criptograf%C3%ADa%20Sim%C3%A9trica%20-%20Soluciones.pdf) (14 páginas, llegaron el 08/09) · Teoría: [[clase-02-cifrado|Clase 02 — Cifrado simétrico]]
> **Enunciado y resolución en la misma página:** debajo de cada ejercicio hay un aviso plegado con la resolución completa. Se abre con un clic, así se puede intentar el ejercicio antes de ver la cuenta. Cada resolución está además **contrastada con la de la cátedra**: coinciden en los ocho, ver [[#Contraste con las soluciones de la cátedra|Contraste con las soluciones de la cátedra]].
...
El [PDF de soluciones](../../raw/guias/guia2/Guia%202%20-%20Criptograf%C3%ADa%20Sim%C3%A9trica%20-%20Soluciones.pdf) apareció en `raw/` el **08/09**, dos semanas después de que esta nota se resolviera, y se contrastó el 14/09. Tiene 14 páginas, dice **2026** en el encabezado, y **coincide con esta nota en los ocho ejercicios**: mismos veredictos, mismos números, y en el Ej. 3b **la misma respuesta no obvia**, $26!$ con los anagramas del alfabeto. Cinco de sus catorce páginas son figuras —las cuatro láminas de `CFB` de la Práctica 03 con la resolución marcada, y los diagramas de `DES` del apunte de Sánchez Arriazu— que la extracción de texto no devuelve y se leyeron sobre el render.

| Ej. | Qué hace la cátedra | Coincide | Qué agrega, qué omite, qué precisar |
|---|---|---|---|
| 1 | Confirma que *"corresponde al ejemplo 2 del apunte Probabilidad y Criptografía"* y resuelve **sólo con el juego `Eav`**, con el adversario *"si el cifrado es 1, $b' = 0$; si no, $b' = 1$"*: $\Pr = \tfrac12\cdot\tfrac12 + \tfrac12\cdot 1 = 0{,}75$ | **Sí** — $3/4$, el mismo valor que la estrategia óptima de la Manera 4 | **No da las "cuatro maneras" que pide su propio enunciado**: las tres distribucionales quedan implícitas en el apunte que cita. Su adversario es más simple que el nuestro y llega al mismo $3/4$, porque cuando se cifró $b$ nunca sale un 1. Las fórmulas escriben $\mathsf{PrivK}^{\mathsf{CPA}}$ donde el texto dice *eav*: errata de tipeo |
| 2 | *"No se cumple la afirmación"*, con el contraejemplo $\mathcal{M} = \{a,b\}$, $\Pr[a] = 0{,}25$, $\Pr[b] = 0{,}75$, dos claves uniformes y la tabla $a \mapsto c/d$, $b \mapsto d/c$; calcula $\Pr[M{=}a \mid C{=}y] = 0{,}25 \ne 0{,}75 = \Pr[M{=}b \mid C{=}y]$ | **Sí** — es el OTP de un bit de esta nota con los símbolos renombrados, y la misma distribución sesgada | Mismo argumento en dos renglones: si valiera, $\Pr[M{=}m] = \Pr[M{=}m']$ para todo par, *"pero no necesariamente esas probabilidades son iguales"* |
| 3 | a) $\Pr[K = (c-m) \bmod 26] = 1/26$; b) recorre mensajes de 1 letra (perfecto), de 2 letras con repetidas (**no**: $\Pr[M{=}aa \mid C{=}aa] = 1/26 \ne 1/26^{2}$, y el `Eav` con $m_0 = aa$, $m_1 = ab$), de 2 letras distintas (perfecto, $24!$ claves por par), y concluye **$26!$ con las palabras de longitud 26 sin letras repetidas**; c) *"la clave debería ser de por lo menos longitud $t$"* | **Sí, en los tres**, y el b) es el resultado que menos se esperaba coincidir | El b) llega por inducción sobre la longitud a la misma construcción de los anagramas. El c) es una línea; esta nota agrega las otras dos condiciones —clave uniforme y **un solo uso**— sin las cuales el período $t$ no alcanza |
| 4 | a) $t = 2$, $k = \texttt{da}$, $m = \texttt{bar} \to c = \texttt{eau}$; b) $\tfrac12(\tfrac13 + \tfrac13\tfrac{26}{26^{2}} + \tfrac13\tfrac{26^{2}}{26^{3}}) = 0{,}17945$ y $\tfrac12(\tfrac13 + \tfrac13\tfrac{25}{26} + \tfrac13\tfrac{25}{26}) = 0{,}48715$, total $0{,}6666$; c) no hay secreto perfecto | **Sí** — $0{,}17945 = \tfrac12\cdot\tfrac{14}{39}$ y $0{,}48715 = \tfrac12\cdot\tfrac{38}{39}$, o sea el $2/3$ exacto | Cuenta por conteo de claves donde esta nota cuenta por la variable $D = k_2 - k_1$; para $m_1$ con $t = 2$ observa que falla *"cuando los símbolos de $k$ son consecutivos, $k_1 = k_2 + 1$"* (26 casos), que es la misma condición $D = x_1 - x_2$. El ejemplo del a) está verificado: $\texttt{b}+\texttt{d} = \texttt{e}$, $\texttt{a}+\texttt{a} = \texttt{a}$, $\texttt{r}+\texttt{d} = \texttt{u}$ |
| 5 | Adversarios **de distinción** en $\mathsf{PrivK}^{\mathsf{CPA}}$: en sustitución, pedir el cifrado de los símbolos y comparar (largo 1), o $m_0 = aa$, $m_1 = ab$ y mirar si el criptograma repite símbolo (largo 2); en Vigenère, pedir $a, aa, aaa, \ldots$ hasta que $c_0 = c_n$ para leer el período $n$, y después $m_0 = a^{2n}$, $m_1 = a^{n}b^{n}$. Cierra con *"ningún esquema determinístico será seguro frente a CPA"* (Katz 3.5, pág. 83) | **Sí** en el veredicto | Dos caminos al mismo lugar: la cátedra **distingue**, esta nota **recupera la clave** con una consulta, que es más fuerte. Su ataque de largo 2 a la sustitución ni siquiera usa el oráculo —es la fuga de $aa$ del Ej. 3b—, y su lectura del período de Vigenère con $a^{k}$ es la misma que la de arriba. Errata: la conclusión del caso de largo 2 repite *"para mensajes de longitud 1"* |
| 6 | a) *"se propaga hasta el final"*; b) *"a $P_1$ y $P_2$"*; c) *"el bloque actual y los $\lceil n/r \rceil$ siguientes"* (Menezes cap. 7), con las **cuatro láminas de `CFB`** de la Práctica 03 y la resolución **marcada encima**: con $c_1$ corrupto, $m_1$ a $m_5$ mal y *"a partir de $m_6$ se corrige"*; con $c_2, c_1$ intercambiados, $m_1$ a $m_6$ mal y *"a partir de $m_7$ se corrige"* | **Sí** — $\lceil n/r\rceil$ es el $n/s$ de esta nota, y $1 + 32/8 = 5$ | **Cierra un cabo de esta nota**: las láminas de `Clase 3 - Modo CFB.pdf` que el 24/08 quedaron *"planteadas sin resolver"* ahora tienen resolución de la cátedra, y coincide con la de [[practica-03-seudoaleatoriedad-y-modos#9. Modo CFB: cuatro láminas con bloque de 32 bits y segmento de 8\|Práctica 03 §9]]: 5 segmentos perdidos por un error de bit, 6 por el intercambio. Su c) habla de *"un error en un bloque de cifrado"* que afecta *"en la encripción"*: es la propagación por realimentación, no un error en el plano como el a) |
| 7 | a) 5 bits; $\varphi(32) = \varphi(2^{5}) = 16$, con el ejemplo $K = 2$, $M = 16 \to 32 \equiv 0$; b) $13 - 4 - 18 - 13 - 7$ paso a paso; c) *"dividir por 7 módulo 32 es multiplicar por 23"*, $K^{-1} = 23$, y recupera $24, 17, 26, \ldots$ | **Sí**, número por número | Justifica el espacio efectivo con la no inyectividad ($M\cdot K \equiv 0$ para $M \ne 0$), que es el mismo argumento que el de la biyección de arriba visto por el contraejemplo. No comenta la coincidencia $C_1 = C_4$ ni da la corrida de Euclides |
| 8 | Feistel según Menezes; el key schedule con la **Figura 2 del apunte de Sánchez Arriazu**; *"si la clave no se modifica en las rondas, se aplica 16 veces la misma; por eso otras claves débiles son $\{0\}^{28}\{1\}^{28}$ y $\{1\}^{28}\{0\}^{28}$"*; la **tabla 7.5 de Menezes** con las cuatro claves en hexadecimal; PC-1 bit por bit, y el ejemplo `E0E0E0E0F1F1F1F1` $\to$ $C$ todo unos, $D$ todo ceros | **Sí** — las cuatro claves y su asignación $C_0/D_0$ son las de la tabla de arriba, y el ejemplo `E0E0…F1F1` es la cuarta fila | Confirma que el apunte `des.pdf` del vault —*"Ver DES (Descripción del algoritmo – Jorge Sánchez Arriazu)"*— es la referencia oficial del tema, y **reproduce su Figura 2 con la errata *"PC-1 (54 bits)"*** que [[des-descripcion-del-algoritmo#10. Erratas y descuidos de la fuente\|Descripción del algoritmo DES]] ya tenía marcada: son 56. Menciona la tabla 7.5 completa, que trae también las semi-débiles, sin desarrollarlas |

**Lo que el contraste deja.** Los ocho resultados quedan **validados por una fuente independiente**, y en el ejercicio donde más se podía haber errado —el $26!$ del Ej. 3b— la cátedra llega exactamente a la misma construcción. Lo que esta nota tiene de más —las cuatro maneras del Ej. 1, la afirmación verdadera que se parece a la falsa del Ej. 2, la recuperación de clave del Ej. 5, la tabla por tamaño de bloque del Ej. 6c, el detalle de por qué PC-1 produce `1F…0E` en el Ej. 8— no está en el PDF y no lo contradice. Y dos cosas que el PDF tiene y esta nota no tenía: la resolución oficial de las láminas de `CFB`, y la confirmación de que **la cátedra resuelve `DES` con el apunte de Sánchez Arriazu y con Menezes**, no con Katz.
...
>
> Cada resolución va **plegada debajo del enunciado** correspondiente, y arranca con el resumen en cursiva de lo que se pide. Los apuntes crudos de los que salieron las cuentas están en [Resolucion Guia 2.md](../../raw/guias/guia2/Resolucion%20Guia%202.md). Y cada una cierra con un párrafo de **contraste con la solución de la cátedra**, cuyo PDF llegó el 08/09 —dos semanas después de resolver— y coincide en los ocho ejercicios.
>
...
>
> **Contraste con la solución de la cátedra.** El PDF de soluciones abre con *"Corresponde al ejemplo 2 del apunte Probabilidad y Criptografía"* —la observación de arriba, confirmada— y resuelve **sólo con el juego `Eav`**, sin hacer las cuatro maneras que su propio enunciado pide (las distribucionales quedan implícitas en el apunte). Su adversario es más simple que el de la Manera 4: *"al recibir un cifrado igual a 1, elige $b' = 0$; caso contrario, $b' = 1$"*. Con $b = 0$ acierta sólo si sale $k_1$, o sea la mitad de las veces; con $b = 1$ **acierta siempre**, porque cifrar $b$ nunca da 1. Total $\tfrac12\cdot\tfrac12 + \tfrac12\cdot 1 = 0{,}75$: el mismo $3/4$, y es coherente con la cota de arriba —el adversario óptimo no puede superar $3/4$, y éste lo alcanza con una regla de un solo criptograma—. Dos detalles de forma: sus fórmulas escriben $\mathsf{PrivK}^{\mathsf{CPA}}_{A,\pi}$ donde el texto dice *eav* (errata de tipeo), y concluye *"como el experimento tiene éxito con probabilidad mayor que $0{,}5$, el esquema no es seguro"*.
...
>
> **Contraste con la solución de la cátedra.** Mismo veredicto —*"no se cumple la afirmación"*— y el mismo argumento en dos renglones: si valiera, $\Pr[M{=}m]$ tendría que ser igual a $\Pr[M{=}m']$, *"pero no necesariamente esas probabilidades son iguales"*. Su contraejemplo es el de arriba con los símbolos cambiados: $\mathcal{M} = \{a, b\}$ con $0{,}25$ y $0{,}75$, dos claves uniformes y la tabla $a \mapsto c, d$ / $b \mapsto d, c$ —un cuadrado latino de orden 2, o sea el OTP de un bit—; calcula $\Pr[C{=}c] = \Pr[C{=}d] = 0{,}5$, verifica el secreto perfecto por las dos caracterizaciones, y exhibe $\Pr[M{=}a \mid C{=}y] = 0{,}25 \ne 0{,}75 = \Pr[M{=}b \mid C{=}y]$. Cita la Definición 2.1 de Katz.
...
>
> **Contraste con la solución de la cátedra.** Coincide en los tres incisos, y **en el b) llega a la misma respuesta no obvia**: *"el mayor tamaño que puede tener el espacio de textos planos para tener secreto perfecto es $26!$, y sólo deberán estar las palabras de longitud 26 donde cada símbolo se usa una sola vez"*. El camino es distinto y vale conocerlo, porque es inductivo: primero mensajes de una letra (perfecto, $25!$ claves por par y $\Pr[M{=}x \mid C{=}y] = 1/26$), después todas las cadenas de dos letras (**no** perfecto: $\Pr[M{=}aa \mid C{=}aa] = 1/26 \ne 1/26^{2}$, y el `Eav` con $m_0 = aa$, $m_1 = ab$ *"detecta que no hay secreto perfecto"*), después las de dos letras **distintas** (perfecto de nuevo, $24!$ claves por par sobre $26\cdot25$ mensajes), y *"extendiendo esta idea"* a las de 26 letras distintas. Es la tabla del matiz de arriba recorrida fila por fila. En el a) hace la cuenta idéntica, *"similar a la que ofrece Katz para el One Time Pad"*. En el c) se limita a *"la clave debería ser de por lo menos longitud $t$; se puede hacer un análisis similar"*: la nota agrega las otras dos condiciones, clave uniforme y un solo uso, sin las cuales el período $t$ no alcanza.
...
>
> **Contraste con la solución de la cátedra.** Coincide número por número. Su ejemplo del a): $t = 2$, $k = \texttt{da}$, $m = \texttt{bar}$, clave extendida $\texttt{dad}$, $c = \texttt{eau}$ —verificado: $\texttt{b}+\texttt{d} = 1+3 = 4 = \texttt{e}$, $\texttt{a}+\texttt{a} = \texttt{a}$, $\texttt{r}+\texttt{d} = 17+3 = 20 = \texttt{u}$—. En el b) cuenta claves en vez de usar la variable $D$: para $m_0$ acierta en 26 de 26 claves de largo 1, 26 de $26^{2}$ de largo 2 y $26^{2}$ de $26^{3}$ de largo 3, o sea $\tfrac12(\tfrac13 + \tfrac13\cdot\tfrac1{26} + \tfrac13\cdot\tfrac1{26}) = 0{,}17945$; para $m_1$ falla *"cuando los símbolos de $k$ son consecutivos, $k_1 = k_2 + 1$"* —26 claves de $26^{2}$, que es la condición $D = x_1 - x_2$ de arriba— y da $0{,}48715$; total $0{,}6666$. Son exactamente $\tfrac12\cdot\tfrac{14}{39}$ y $\tfrac12\cdot\tfrac{38}{39}$. El c) cierra igual: *"como la probabilidad no es $0{,}5$, el esquema no tiene secreto perfecto"*.
...
>
> **Contraste con la solución de la cátedra.** Mismo veredicto por otro camino: la cátedra construye adversarios **de distinción** dentro de $\mathsf{PrivK}^{\mathsf{CPA}}$, no recupera la clave. Para la sustitución con mensajes de una letra, pide al oráculo el cifrado de todos los símbolos —incluidos $m_0$ y $m_1$— y compara; para dos letras, emite $m_0 = aa$ y $m_1 = ab$ y mira si el criptograma repite símbolo, **sin usar el oráculo**: es la fuga del Ej. 3b, que ya rompe `Eav`. Para Vigenère, pide $a, aa, aaa, \ldots$ hasta ver $c_0 = c_n$ y así lee el período $n$ —la misma lectura del período de arriba—, y después emite $m_0 = a^{2n}$ y $m_1 = a^{n}b^{n}$, que se distinguen por la forma del criptograma. Cierra con la conclusión general, citando *"Katz cap. 3.5, pág. 83"*: **ningún esquema determinístico es seguro frente a `CPA`**, porque el adversario pide $\mathsf{Enc}_k(m_0)$ y $\mathsf{Enc}_k(m_1)$ y compara. Las dos resoluciones son válidas; la de arriba prueba más (recupera $k$), la de la cátedra se queda en lo que el juego pide. Errata del PDF: la conclusión del caso de dos letras repite *"para mensajes de longitud 1"*.
...
>
> **El $n$ que dibuja la cátedra es 32, no 64.** Las cuatro láminas de [`Clase 3 - Modo CFB.pdf`](../../raw/practicas/Clase%203%20-%20Modo%20CFB.pdf) —material propio de la cátedra, del 24/08— trabajan con $n = 32$ y $s = 8$: las **tres primeras** lo llevan rotulado *"Ejemplo: n = 32; s = 8"*, y la cuarta, la del ejercicio abierto, no lo repite. Con esos parámetros la cuenta da $1 + 32/8 = \mathbf{5}$ caracteres: **es la primera fila de la tabla, y conviene tenerla a mano porque es el número que sale de su propio material.** El recorrido de las láminas está en [[practica-03-seudoaleatoriedad-y-modos|Práctica 3 — Seudoaleatoriedad y modos]].
>
> Esas mismas láminas plantean además **otros dos modos de falla que este ejercicio no pregunta**: bloques que llegan **fuera de orden** (la lámina 3 muestra $c_2, c_1, c_3, c_4, \dots$) y un ejercicio abierto —*"analizar"*— con $c_1$ corrupto. Las láminas del 24/08 los dejaban planteados sin resolver; **el PDF de soluciones de la cátedra los resuelve**, con la respuesta dibujada encima de las mismas láminas: con $c_1$ corrupto, $m_1$ a $m_5$ mal y *"a partir de $m_6$ se corrige"*; con $c_2, c_1$ intercambiados, $m_1$ a $m_6$ mal y *"a partir de $m_7$ se corrige"*. Coincide con lo que la wiki había resuelto por su cuenta en [[modos-de-encadenamiento#Bloques que llegan fuera de orden: un modo de falla distinto|Modos de encadenamiento]] y en [[practica-03-seudoaleatoriedad-y-modos#9. Modo CFB: cuatro láminas con bloque de 32 bits y segmento de 8|Práctica 03 §9]]. Ojo con la diferencia: un bloque **fuera de orden** no es el error de bit de este inciso, y la autosincronización que se afirma más arriba está establecida sólo para el error de bit.
>
> **Contraste con la solución de la cátedra.** a) *"En CBC un error en un bit se propaga hasta el final"*; b) *"se propaga a $P_1$ y $P_2$"*; c) *"afecta el bloque actual y los $\lceil n/r\rceil$ bloques siguientes"*, citando las propiedades del modo `CFB` de Menezes, cap. 7: con $r$ el tamaño del segmento, es el $1 + n/s$ de arriba. Y trae lo que a esta nota le faltaba: las cuatro láminas de `Clase 3 - Modo CFB.pdf` **con la resolución dibujada** — $m_1$ a $m_5$ mal y *"a partir de $m_6$ se corrige"* para el $c_1$ corrupto, o sea 5 segmentos con $n = 32$ y $s = 8$; y para los bloques mezclados $c_2, c_1$, seis mal y *"a partir de $m_7$ se corrige"*. Las dos coinciden con la simulación de la Práctica 03. Una redacción a leer con cuidado: su c) dice que un error *"en un bloque de cifrado"* afecta *"en la encripción"* al bloque actual y los siguientes; se refiere a la realimentación del criptograma en el registro durante el cifrado, no a un error en el plano como el del a).
...
>
> **Contraste con la solución de la cátedra.** Idéntica en los tres incisos: *"5 bits, para poder encriptar del 0 al 31; el espacio efectivo de la clave es $\varphi(32) = \varphi(2^{5}) = 16$"*, justificado por el contraejemplo *"si $K = 2$ y $M = 16$, $E(2, M) = 32 = 0$ módulo 32"* —la no inyectividad que arriba se argumenta por la biyección—; el cifrado $13 - 4 - 18 - 13 - 7$ desplegado paso a paso con los mismos xor; y el descifrado con *"dividir por 7 módulo 32 es lo mismo que multiplicar por 23"*, $K \cdot K^{-1} \equiv 1$, recuperando $24, 17, 26, \ldots$ hasta el tercer bloque. No comenta la coincidencia $C_1 = C_4$ ni corre Euclides para obtener el 23.
...
>
> **Contraste con la solución de la cátedra.** El mismo mecanismo en dos piezas —Feistel con Menezes, cap. 7, y el key schedule con la **Figura 2 del apunte de Sánchez Arriazu**— y la misma conclusión: *"si la clave utilizada no se modifica en las sucesivas rondas, se aplicará 16 veces la misma clave al encriptar que al desencriptar; es decir, los 16 rounds inversos son iguales a los 16 originales. Por eso, otras claves débiles son $\{0\}^{28}\{1\}^{28}$ y $\{1\}^{28}\{0\}^{28}$"*: las dos filas mixtas de arriba. Reproduce la **tabla 7.5 de Menezes** con las cuatro claves en hexadecimal —`0101…`, `FEFE…`, `1F1F1F1F0E0E0E0E`, `E0E0E0E0F1F1F1F1`— con la misma asignación de $C_0$ y $D_0$ que la tabla de esta nota, y desarrolla PC-1 bit por bit para mostrar que `E0E0E0E0F1F1F1F1` deja *"el bloque C con todos bits en 1 y el bloque D con todos bits en 0"*, que es la cuarta fila. Tres cosas más: la aclaración *"estamos considerando las claves de 56 bits, lo que se obtiene después de PC-1"*, que es la sección de los bits de paridad; la remisión explícita al apunte *"DES (Descripción del algoritmo – Jorge Sánchez Arriazu)"*, que confirma a [[des-descripcion-del-algoritmo|des.pdf]] como la referencia oficial de la cátedra para esto; y **la errata heredada**: la Figura 2 que copia del apunte rotula PC-1 como *"(54 bits)"*, y son 56 — ya estaba marcada en [[des-descripcion-del-algoritmo#10. Erratas y descuidos de la fuente|Erratas y descuidos de la fuente]]. Las semi-débiles las nombra vía la tabla 7.5, sin desarrollarlas.
