...
import type { Page, PageMeta, SubjectConfig } from "@sinapsis/contract";
 ...
 */
export function resolveConfig(row: SubjectRow): SubjectConfig {
...
/**
 * Predicado "este tipo de página cuenta como contenido".
 * Solo quedan fuera los tipos declarados con `countsAsContent: false`; un tipo
 * que el config no declara cuenta (y además genera un warning en el sync).
 */
export function contentTypePredicate(config: SubjectConfig): (type: string) => boolean {
